...
- **`operate-runtime`** — Phase 3 P1 serving keystone: composes a
  ...
  openApiRouteDefinition GET /v1/openapi.json + buildOpenApiHandler), compile
...
- **`operate-web`** — Phase 3 P3.1: a framework-neutral,
  ...
  P3.27/ADR-0182 describe = describeWebApi(manifest, viewer) → a per-caller
  WebApiDescriptor projected from compileWebApp — each entity's available view
  kinds mapped to concrete /ui/... route paths + the global /ui/app + /ui/_describe
  routes; pure data, the parity sibling of operate-runtime's api-descriptor):

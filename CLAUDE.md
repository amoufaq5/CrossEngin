...
P2.14 + P1.23** landed: **60 packages + 3 apps, 124 meta-schema
tables, 6,520 offline tests + 15 gated real-Postgres integration
tests (10 worker + 5 operate-server)**, all green, no type errors.
...
(P2.11) → page (P2.13). **P2.14 (ADR-0118) added a projection
...
- **`apps/operate-server`** — Phase 3 P1.7 + P1.9: the runnable
  ...
  per-caller redaction, RBAC, and keyset pagination end-to-end.
- **`apps/workflow-worker`** — Phase 3 P2.3: the runnable
  ...
  PostgresWorkerHeartbeatStore.listAll → an onIncident sink). All logic
  offline-tested.

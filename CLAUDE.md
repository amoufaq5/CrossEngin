...
P2.14 + P1.23 + P1.24 + P1.25 + P2.15 + P1.26 + P2.16** landed: **60
packages + 3 apps, 124 meta-schema tables, 6,524 offline tests + 23
gated real-Postgres integration tests (12 worker + 11 operate-server)**,
...
`onIncident` (else log-only). The heartbeat loop now ends in a durable
record: write → detect → plan → run → persist. **P2.14 (ADR-0118) added a projection

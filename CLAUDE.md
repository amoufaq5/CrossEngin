...
- **`active-active-runtime-pg`** — Phase 3 P6.4: the Postgres sibling for the
  ...
  shifted `rootDir` to `.` (`dist/src` + `dist/bin`). A recurring writer (worker
  mode/CLI draining an engine's events) is the follow-up.

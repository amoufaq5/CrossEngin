...
P2.5 + P2.6 + P2.7 + P2.8 + P2.9 + P2.10** landed: **60 packages + 3
apps, 124 meta-schema tables, 6,491 offline tests + 7 gated
real-Postgres integration tests**, all green, no type errors.
...
timeoutInstance · executeActivity · timeoutActivity). **The P2
distributed-worker arc is complete.** P1 added
...
- **`workflow-worker`** — Phase 3 P2 + P2.1 + P2.2 + P2.5 + P2.7 +
  P2.8 + P2.10: the distributed worker. 13 modules: lock-key (advisoryLockKey(namespace) → a stable signed
...
- **`apps/workflow-worker`** — Phase 3 P2.3: the runnable
  ...
  all→claim+retry+timeout+activity-timeout+execute; each

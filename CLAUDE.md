...
P1.27 + P1.28 + P2.18 + P2.19 + P2.20 + P2.21 + P2.22** landed: **61 packages + 3 apps, 124
meta-schema tables, 6,574 offline tests + 26 gated real-Postgres
integration tests (15 worker + 11 operate-server)**, all green, no type
errors. (Earlier
...
incident audit trail is now operable from the shell. **P2.14 (ADR-0118) added a projection
...
- **`apps/workflow-worker`** — Phase 3 P2.3: the runnable
  ...
  through on declaration + the escalation's pages on escalation. P2.21
  ...
  operator subcommand (parseIncidentsArgs + runIncidents over a structural
  IncidentQuerySource; executeIncidents wires the conn; verify exits 1 on
  drift for CI). All logic
  offline-tested. An ops guide (README.md) documents the 8 modes,

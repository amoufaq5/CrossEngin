...
P2.14 + P1.23 + P1.24 + P1.25 + P2.15 + P1.26** landed: **60 packages
+ 3 apps, 124 meta-schema tables, 6,521 offline tests + 22 gated
real-Postgres integration tests (11 worker + 11 operate-server)**, all
...
stale-worker incident (the `onIncident` sink is the seam for real
paging / persistence). The heartbeat loop now runs live end-to-end:
write → detect → plan → run. **P2.14 (ADR-0118) added a projection

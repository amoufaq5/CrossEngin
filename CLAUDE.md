...
M2.X.5.aa.z.15 + M2.X.5.aa.z.16 + M2.X.5.aa.z.17 + M2.X.5.aa.z.18 + M2.X.5.aa.z.19 + M2.X.5.aa.z.20 + M2.X.5.aa.z.21 + M2.X.5.aa.z.22 + M2.X.5.aa.z.23 + M2.X.5.aa.z.24 + M2.X.5.aa.z.25 + M2.X.5.aa.z.26 + M2.X.5.aa.z.27 + M2.X.5.aa.z.28 + M2.X.5.aa.z.29 + M2.X.5.aa.z.30 + M2.X.6 + M2.X.11 + M2.X.11.x + M2.X.12 + M2.X.13 + M2.X.14 + M2.X.15 + M2.X.16 + M5.10.5 + M6.6.x + M6.6.y + M6.7 + M6.7.x + M6.7.y + M6.7.z + M6.7.z.embed + M6.7.zz + M6.7.zz.dry-run + M6.7.zz.tenant + M6.7.zz.tenant.dashboard + M6.7.zz.tenant.opt-out + M6.7.zz.tenant.opt-out.reason + M6.7.zz.tenant.opt-out.expiry + M6.7.zz.tenant.opt-out.alerts + M6.7.zz.tenant.opt-out.cli + M6.7.zz.tenant.opt-out.cli.effective + M6.7.zz.tenant.opt-out.cli.mutate + M6.7.zz.tenant.opt-out.cli.list + M6.7.zz.tenant.retention-set + M6.7.zz.tenant.retention-delete + M6.7.zz.tenant.opt-out.history + M6.7.zz.tenant.opt-out.cli.restore + M6.7.zz.tenant.opt-out.history-retention + M6.7.zz.tenant.opt-out.cli.diff-history + M6.7.zz.tenant.opt-out.cli.prune + M6.7.zz.tenant.opt-out.cli.history.cursor + M6.7.zz.tenant.opt-out.cli.restore.dry-run + M6.7.zz.tenant.batch + M6.7.zz.tenant.opt-out.cli.diff + M6.7.zz.tenant.opt-out.cli.diff.vs-platform + M6.7.zz.tenant.opt-out.cli.diff.cross-table + M6.7.zz.tenant.opt-out.cli.diff.exit-on-divergence + M6.7.zz.tenant.opt-out.cli.effective-batch + M6.7.zz.tenant.opt-out.cli.diff.add-tenant + M6.7.zz.tenant.opt-out.cli.diff.threshold + M6.7.zz.tenant.opt-out.history.actor-join + M6.7.zz.tenant.opt-out.cli.history.actor-filter + M6.7.zz.tenant.opt-out.cli.history.attributes + M6.7.zz.tenant.opt-out.cli.diff.add-table + M6.7.zz.tenant.opt-out.cli.diff-timeline + M6.7.zz.tenant.opt-out.cli.diff-timeline.with-actor-names + M6.7.zz.tenant.opt-out.cli.diff-timeline.add-tenant + M6.7.zz.tenant.opt-out.cli.diff-timeline.cross-table + M6.7.zz.tenant.opt-out.cli.diff-timeline.actor-filter + M6.7.zz.tenant.opt-out.cli.diff-timeline.kind-filter + M6.7.zz.tenant.opt-out.cli.diff-timeline.cursor + M6.7.zz.tenant.opt-out.history.before-id + M6.7.zz.tenant.opt-out.cli.diff-timeline.before-id + M6.7.zz.tenant.opt-out.cli.diff-history.kind-filter + M6.7.zz.tenant.opt-out.cli.diff-timeline.multi-actor + M6.7.zz.tenant.opt-out.cli.diff-timeline.multi-kind + M6.7.zz.tenant.opt-out.cli.history.range + M6.7.zz.tenant.opt-out.cli.diff-timeline.range + M6.7.zz.tenant.opt-out.cli.diff-history.actor-filter + M6.7.zz.tenant.opt-out.cli.diff-history.with-actor-names + M6.7.zz.tenant.opt-out.cli.diff-history.actor-not + M6.7.zz.tenant.opt-out.cli.history.actor-not + M6.7.zz.tenant.opt-out.cli.diff-timeline.actor-not + M6.7.zz.tenant.opt-out.cli.diff-history.kind-not + M6.7.zz.tenant.opt-out.cli.history.system-only + M6.7.zz.tenant.opt-out.cli.history.actor-not.multi + M6.7.zz.tenant.opt-out.cli.history.actor-filter.multi + M6.7.zz.tenant.opt-out.cli.diff-history.system-only + M6.7.zz.tenant.opt-out.cli.diff-timeline.system-only + M6.7.zz.tenant.opt-out.cli.diff-history.kind-not.multi + M6.7.zz.tenant.opt-out.cli.diff-history.per-side + M6.7.zz.tenant.opt-out.cli.diff-history.per-side.system-only + M6.8 + M6.8.x + M6.8.x.trace + M6.8.y + M8 + M8.1 + M8.2 +
...
**56 packages + 1 app, 129 meta-schema tables, 9,068 tests**,
all green, no type errors. M6.7.zz.tenant.opt-out.cli.diff-history.per-side.system-only
closes ADR-0212 Q2 + ADR-0215 deferred per-side actor-presence
boolean-pair by adding 4 per-side actor-presence flags
(--system-only-a, --no-system-a, --system-only-b, --no-system-b)
to retention diff-history. This is the 5th and final per-side
future Q completing the per-side expectation-check matrix on the
diff-history surface. The 4-flag intra-side mutual-exclusivity
design space resolves cleanly by reusing the ActorPresenceFilter
discriminated string union from ADR-0209/0212 per side — same
type at adapter level, CLI handles 4 booleans + intra-side
mutual exclusivity check + translates to 2 union values one per
side. Real asymmetric use cases — migration-to-human handoff
verification (A is system migration, B is human adjustment),
human-to-automation handoff (A is manual opt-out, B is system
sweep), system sweep audit pair (both sides system), operator
forensic discipline (both sides human), compose with kind
expectations (A is human opt_out_set + B is system
policy_deleted), compose with actor expectations (A is Alice's
human + B is system NOT Bob), symmetric assertion via per-side
flags equivalent to global but explicit when composing.
DiffHistoryEntriesInput gains 2 new optional fields
actorPresenceA?: ActorPresenceFilter + actorPresenceB?:
ActorPresenceFilter (ADDITIVE matching ADR-0215 precedent —
per-side semantically distinct from global not just renames;
global symmetric assertion stays for operators wanting "both
events" semantic). Adapter gains 2 new conditional blocks
positioned immediately after the existing global actorPresence
block (end of actor dimension chain) — single-side check (not
"both events" loop) throws with explicit "event A" / "event B"
prefix using established asymmetric rendering (positive shows
actual UUID since operator needs to see WHO authored event;
negative renders <system> placeholder since operator knows
excluded value). Check ordering — global actorPresence fires
first then per-side A then per-side B matching ADR-0215
ordering pattern. CLI parses 4 new boolean flags via
getBooleanFlag + 2 intra-side mutual-exclusivity checks (both
A flags set exit 2; both B flags set exit 2; A's and B's flags
don't interact — operators can set --system-only-a +
--no-system-b for asymmetric "A is system, B is human"
assertion canonical use case) + translates to 2 actorPresenceA/
actorPresenceB union values + threads to adapter. JSON envelope
gains 4 new boolean fields (systemOnlyA, noSystemA, systemOnlyB,
noSystemB) matching CLI flag names verbatim using established
boolean-echo convention from ADR-0209/0212 — operators read
literal flag state; intra-side mutual exclusivity at CLI
guarantees both never true simultaneously. Composition with
global — per-side composes freely; when both set BOTH check
fire global first (matches ADR-0205/0215 contradictory-
combination ordering). --system-only + --system-only-a is
redundant but harmless; --system-only + --no-system-a is
contradictory at data layer (global fires first surfacing
"expected both events to be system-authored" error);
--system-only-a + --no-system-b is the canonical asymmetric
pattern; --system-only-a + --system-only-b is equivalent to
global but explicit. cli.ts helpText extended for retention
diff-history usage line with [--system-only-a | --no-system-a]
[--system-only-b | --no-system-b] notation + 4-line description
explaining per-side actor-presence + intra-side mutual
exclusivity + A's-and-B's-flags-don't-interact + per-side-
fires-after-global semantic. 10 new adapter tests in a new
"PostgresTraceRetention.diffHistoryEntries per-side
actorPresence expectation checks" describe block — --system-
only-a accepts/throws-with-actual-UUID, --no-system-a accepts/
throws-with-<system>-rendering, --system-only-b throws (B
independent of A), --no-system-b throws with <system>, canonical
asymmetric pattern --system-only-a + --no-system-b accepts when
A=null + B=UUID, composition global --system-only fires BEFORE
per-side --no-system-a surfacing global error first, omits per-
side actor-presence checks when fields not set backward compat,
per-side A fires BEFORE per-side B in check order. 13 new CLI
tests in a new "runRetention diff-history per-side --system-
only / --no-system" describe block — exit 2 when --system-only-a
+ --no-system-a both set with explicit error, exit 2 when
--system-only-b + --no-system-b both set, --system-only-a +
--no-system-b allowed (different sides asymmetric assertion),
threads actorPresenceA/B='system_only'/'no_system' for each
flag, omits per-side actorPresence fields when neither set
backward compat, composes with global --system-only + per-side
--no-system-a both threaded global fires first at adapter, JSON
envelope echoes systemOnlyA + noSystemA + systemOnlyB +
noSystemB booleans, JSON envelope all per-side actor-presence
booleans false when none set, adapter per-side error propagates
exit 1 with per-side error format, intra-side mutual exclusivity
check fires BEFORE PG adapter call (capture length 0). Test
count: 9,045 → 9,068 (+23: adapter +10, CLI +13). ADR-0216
documents the design + 8 rejected alternatives + 6 future Qs
(per-side --system-only-X + per-side --actor-id-X contradiction
detection defer, JSON envelope unification to discriminated
string per side defer would be breaking, apply per-side actor-
presence to retention diff-timeline N/A list-style not per-
event-pair, apply per-side to retention history N/A list-style,
short-circuit LEFT JOIN when actorPresenceA='system_only'
optimization pairs with ADR-0212 Q3 + ADR-0213 Q5 family defer,
combined per-side actor-presence + actor-id consistency check
operators can express via not setting --actor-id-a defer). The
retention diff-history surface now has the COMPLETE per-side
expectation-check matrix — 5 dimensions × 3 modes (global
symmetric + per-side A + per-side B) = 15 expectation-check
flag families plus --with-actor-names: event_kind positive
(--kind ADR-0198 + --kind-a ADR-0215 + --kind-b ADR-0215),
event_kind negative multi (--kind-not ADR-0214 + --kind-not-a
ADR-0215 + --kind-not-b ADR-0215), actor_id positive (--actor-
id ADR-0203 + --actor-id-a ADR-0215 + --actor-id-b ADR-0215),
actor_id negative (--actor-id-not ADR-0205 + --actor-id-not-a
ADR-0215 + --actor-id-not-b ADR-0215), system presence
(--system-only/--no-system ADR-0212 + --system-only-a/
--no-system-a ADR-0216 + --system-only-b/--no-system-b
ADR-0216). Operators have unprecedented control over per-event-
pair forensic assertions — symmetric AND asymmetric
expectations across kind + actor + system-presence dimensions
in a single command; the matrix is now fully symmetric every
dimension has both global symmetric and per-side asymmetric
variants. Natural follow-up milestones — multi-value tuple
expectations on actor-id positive/negative (ADR-0203 Q2 +
ADR-0205 Q1), multi-value tuple expectation on global --kind
positive (ADR-0198 Q2), per-side multi-value variants combining
ADR-0215 per-side pattern with future multi-value tuples. The
retention CLI now has 18 actions with the most comprehensive
multi-flag coverage in the codebase; the per-side asymmetric
expectation-check matrix on diff-history is COMPLETE for the 5
actor + kind dimensions.
M6.7.zz.tenant.opt-out.cli.diff-history.per-side
closes ADR-0198 Q1 + ADR-0203 Q1 + ADR-0205 Q2 + ADR-0208 Q2 by
adding 8 per-side asymmetric expectation-check flags to
`retention diff-history` covering 4 of the 5 per-side future Qs
in one milestone. The 5th future Q (ADR-0212 Q2 --system-only-a/
--system-only-b/--no-system-a/--no-system-b per-side boolean-
pair) deferred since the 4-flag intra-side mutual-exclusivity
design deserves its own treatment. The 8 new flags pair with
their existing global counterparts — --kind-a / --kind-b (closes
ADR-0198 Q1), --kind-not-a / --kind-not-b multi-value following
ADR-0214 (closes ADR-0208 Q2), --actor-id-a / --actor-id-b
(closes ADR-0203 Q1), --actor-id-not-a / --actor-id-not-b
(closes ADR-0205 Q2). Real asymmetric use cases — forensic
state-transition verification (assert A is deletion event + B
is rebuild event in correct order), per-side actor attribution
(assert A is Alice's mutation + B is Bob's), per-side exclusion
with different excluded actors per side (A not migration SA + B
not CI SA), per-side multi-value kind exclusion (A is opt_out
workflow not maintenance + B unconstrained), hybrid global +
per-side belt-and-suspenders (global symmetric assertion +
per-side narrows further), maximum-discipline forensic
assertion combining all per-side variants. ADDITIVE not breaking
rename — DiffHistoryEntriesInput gains 8 new optional fields
(eventKindA, eventKindB, eventKindsNotA, eventKindsNotB,
actorIdA, actorIdB, actorIdNotA, actorIdNotB) preserving the
existing 5 global fields; per-side flags are semantically
DIFFERENT operations from global flags (assert different things);
operators wanting symmetric "both events" assertion continue to
use global; operators wanting asymmetric per-side use new flags;
breaking rename would lose the symmetric semantic. Check
ordering — global before per-side within each pair (kind →
kindA → kindB → kindsNot → kindsNotA → kindsNotB → actorId →
actorIdA → actorIdB → actorIdNot → actorIdNotA → actorIdNotB →
actorPresence). Each per-side check fires AFTER its global
counterpart; operators combining global + per-side see global
error first if global fails (matches ADR-0205 contradictory-
combination ordering). Per-side error message format — positive
shows actual unexpected value `expected event A to have
event_kind 'X' but A is 'Y'` mirroring ADR-0203 rendering;
negative names the side without showing value `expected event A
to have actor_id NOT 'X' but A matches` mirroring ADR-0205;
<system> placeholder for null actor_id preserved across per-
side. CLI parsing — 8 new flag parses in runRetentionDiffHistory
(getStringFlag + isOptOutHistoryEventKind validation for
--kind-a/-b, getMultiFlag + per-occurrence validation loop for
--kind-not-a/-b matching ADR-0214 pattern, getStringFlag for
actor-id-a/-b/-not-a/-not-b). JSON envelope gains 8 new fields
matching CLI flag names verbatim (kindA, kindB, kindsNotA,
kindsNotB, actorIdA, actorIdB, actorIdNotA, actorIdNotB) —
string-or-null or array-or-null shapes following established
conventions; note envelope-level actorIdA (operator's CLI flag)
vs result.actorIdA (actual diff output) are at different JSON
nesting levels operators distinguish via path. Composition with
global flags — per-side composes freely; when both global +
per-side set BOTH checks fire global first; --kind opt_out_set
+ --kind-a opt_out_set is redundant but harmless; --kind
opt_out_set + --kind-a retention_set is contradictory at data
layer (global fires first surfacing global error); --kind-a
opt_out_set + --kind-b opt_out_cleared is canonical asymmetric
pattern. cli.ts helpText extended for retention diff-history
usage line with all 8 new flags grouped by family
(kind/kind-not/actor-id/actor-id-not) + concise descriptions of
per-side semantic + independence from global + error format. 17
new adapter tests in a new "PostgresTraceRetention
.diffHistoryEntries per-side expectation checks (M6.7.zz.tenant
.opt-out.cli.diff-history.per-side)" describe block — --kind-a
accepts/throws-with-actual-kind, --kind-b throws (B independent
of A), --kind-a + --kind-b both pass when respective matches,
--kind-not-a accepts/throws-with-multi-value-list-format,
--kind-not-b throws independently, --actor-id-a accepts/throws-
with-actual-actor, --actor-id-a throws-with-<system>-rendering
for null actor_id, --actor-id-b throws independently, --actor-
id-not-a accepts/throws, --actor-id-not-b throws independently,
composition all 4 per-side flags check independently when their
sides match, composition global --kind fires BEFORE per-side
--kind-a surfacing global error first, omits all per-side
checks when none set backward compat. 17 new CLI tests in a new
"runRetention diff-history per-side expectations" describe
block — --kind-a/-b invalid value exits 2 each with offending
value named, --kind-not-a/-b invalid value exits 2 on FIRST
invalid occurrence each, threads eventKindA/B + eventKindsNotA/B
+ actorIdA/B + actorIdNotA/B as appropriate when each flag set,
omits all per-side fields when no per-side flag set backward
compat, composes with global --kind + per-side --kind-a both
threaded, JSON envelope echoes kindA + kindB + kindsNotA +
kindsNotB + actorIdA + actorIdB + actorIdNotA + actorIdNotB
when set, JSON envelope all per-side fields null when none set,
adapter per-side error propagates exit 1 with per-side error
format. Test count: 9,011 → 9,045 (+34: adapter +17, CLI +17).
ADR-0215 documents the design + 10 rejected alternatives + 6
future Qs (--system-only-a/-b + --no-system-a/-b per-side
boolean-pair closes ADR-0212 Q2 design space needs careful CLI
ergonomics for intra-side mutual exclusivity, per-side multi-
value --actor-id-a repeatable for tuple expectation defer, per-
side --kind-a multi-value for tuple expectation defer, JSON
envelope field name distinction between operator-input
expectations and result diff values future rename, apply per-
side pattern to retention diff-timeline N/A since list-style
not per-event-pair, combined per-side + system actor presence
--system-only-a + --no-system-b for "A is system + B is human"
defer). The retention diff-history surface now has 13
expectation-check flag families forming a 4×3 matrix — 4
dimensions (event_kind positive, event_kind negative, actor_id
positive, actor_id negative) × 3 modes (global symmetric, per-
side A, per-side B) plus the actor-presence pair (--system-
only/--no-system global only); operators have unprecedented
control over per-event-pair forensic assertions with symmetric
+ asymmetric expectations across kind + actor + system-
presence dimensions in a single command. The retention CLI now
has 18 actions with the most comprehensive multi-flag coverage
in the codebase — operators can express virtually any state-
transition assertion in a single command. The natural follow-
up milestones — --system-only-a/--system-only-b/--no-system-a/
--no-system-b per-side boolean-pair (ADR-0212 Q2), multi-value
tuple expectations on per-side variants (ADR-0198 Q2 / ADR-0203
Q2 family).
M6.7.zz.tenant.opt-out.cli.diff-history.kind-not.multi
closes ADR-0208 Q1 by widening `--kind-not <event-kind>` from
single-value to repeated flag (OR-semantic exclusion expectation
check) on `retention diff-history` using the multiFlags
infrastructure from ADR-0183. Operator declares "I expect
NEITHER event to have any of these N event_kinds" and adapter
throws on match with the side(s) named. Mirror of ADR-0210
(history actor-id-not multi-value), ADR-0207 (diff-timeline
actor-id-not multi-value), ADR-0200 (diff-timeline multi-kind)
— same flag-repetition pattern + same one-shot breaking rename
+ same per-occurrence validation against the 4-value enum
matching ADR-0200 pattern (exits 2 on FIRST invalid value with
that value named) + same empty-array-as-filter-not-set
convention. Real cohort-exclusion use cases — workflow audit
excluding maintenance kinds (assert NEITHER event is
policy_deleted OR retention_set), anti-automation pair
verification (assert NEITHER event is a system-automation kind
before treating diff as deliberate human-policy work), CI gate
with kind-list exclusion (fail build if either event matches
any of these 3 kinds), cohort-aware forensic comparison
composing with --system-only from ADR-0212 for layered
assertion. Breaking adapter rename DiffHistoryEntriesInput
.eventKindNot?: OptOutHistoryEventKind → eventKindsNot?:
ReadonlyArray<OptOutHistoryEventKind>; session-recent code
from ADR-0208 with no external consumers contained scope
matches ADR-0199/0207/0210/0211 one-shot break precedent (the
workspace has established the breaking-rename pattern across
the actor + kind family — 6 consecutive single→multi renames
make this milestone mechanical). Adapter rewritten with Set-
based O(1) membership lookup; always-list error format
`event_kind in ['X']` for single-value + `event_kind in
['X', 'Y']` for multi-value (single-value error breaks from
ADR-0208's `event_kind 'X'` shape — acceptable since field
rename is itself breaking; consistent rendering across single
+ multi reads better than separate single-value-special-case
vs multi-value-list-case formats). CLI runRetentionDiffHistory
reads via getMultiFlag("kind-not") instead of getStringFlag;
per-occurrence validation via isOptOutHistoryEventKind loop
matching ADR-0200 multi-kind on diff-timeline pattern — exits
2 on FIRST invalid value with that value named (operators
reading error know exactly which --kind-not occurrence
failed). JSON envelope renamed kindNot: string | null →
kindsNot: string[] | null matching ADR-0207/0210/0211 array-
or-null canonical multi-value envelope shape across the
family. Within-action asymmetry documented — positive --kind
stays single-value while negative --kind-not now multi-value;
documented as future Q ("widen --kind to multi-value tuple
expectation closes ADR-0198 Q2 + restores within-action
symmetry"); deferred to keep milestone scope focused on
closing ADR-0208 Q1 specifically. Asymmetry acceptable
because (1) positive + negative semantically distinct
dimensions — negative path benefits more from multi-value
since "exclude these N noise kinds" is more common than
"include exactly one of these N specific kinds" (operators
with positive multi-value would typically just use no filter
and post-filter), (2) diff-timeline has BOTH multi-value
patterns established, (3) the 4-value event_kind enum makes
multi-value positive less compelling than multi-value
negative since operators wanting "any of these 3 kinds" can
equivalently use "neither of the remaining 1 kind" via
--kind-not (negative is strict superset of positive's
expressiveness). cli.ts helpText updated from [--kind-not
<event-kind>] to [--kind-not <event-kind> ...] indicating
repeatable + description extended explaining "repeatable +
NEITHER event has any of the listed event_kinds + match by
either side on any listed kind exits 1". Adapter test block
rewritten + expanded from 8 to 12 tests under renamed
"diffHistoryEntries --kind-not exclusion check (M6.7.zz
.tenant.opt-out.cli.diff-history.kind-not + .multi)" describe
block — single-value accepts neither matches, single-value
throws A matches with new event_kind in ['X'] format, single-
value throws B matches, single-value throws naming both
sides when neither matches, multi-value accepts when neither
has any of N excluded kinds (NEW), multi-value throws when A
matches one of N kinds with multi-value error format (NEW),
multi-value throws naming both when both events match
different kinds in exclusion list (NEW), omits check when
eventKindsNot not set backward compat, treats empty
eventKindsNot array as filter-not-set (NEW), composes with
--kind expectation check (both pass when distinct kinds),
contradictory --kind + --kind-not surfaces kind check first,
composes with --actor-id-not (both checks fire independently).
CLI test block rewritten + expanded from 7 to 10 tests under
renamed "runRetention diff-history --kind-not (M6.7.zz
.tenant.opt-out.cli.diff-history.kind-not + .multi)" describe
block — exit 2 on invalid --kind-not value, exit 2 on FIRST
invalid occurrence when multiple flags supplied with bogus
value named (NEW), threads single-element array when --kind-
not set once, threads multi-element array when --kind-not
repeated (NEW), omits when not set backward compat, composes
with --kind both threaded independently (multi-value kind-
not), adapter exclusion error propagates exit 1 with multi-
value error format, JSON envelope echoes single-element array,
JSON envelope echoes multi-element array (NEW), JSON envelope
kindsNot=null when not set. Test count: 9,004 → 9,011 (+7
net: adapter +4, CLI +3). ADR-0214 documents the design + 10
rejected alternatives + 6 future Qs (widen --kind to multi-
value tuple expectation closes ADR-0198 Q2 + restores within-
action symmetry, --kind-not @file.txt defer since 4-value
enum is bounded, --kind-not filter on retention history pairs
with ADR-0206 Q4 defer, --kind-not filter on diff-timeline
across all 3 paths pairs with ADR-0207 Q4 + ADR-0200 future
Q defer, CLI-side dedup of duplicates defer Set handles fine,
semantic-shape exclusion grouping write-mutations
--exclude-write-mutations shorthand operator-policy defer).
The retention CLI's multi-value flag coverage continues to
expand — retention history actor + actor-not multi (ADR-
0210/0211), diff-timeline actor + actor-not + kind multi
(ADR-0199/0207/0194/0200), diff-history kind-not multi from
this milestone (the first multi-value expectation check on
diff-history); the symmetric positive --kind multi-value
tuple expectation is the natural follow-up. The retention
CLI now has 18 actions with comprehensive multi-flag coverage
spanning the actor + kind dimensions across all 3 surfaces
with surface-appropriate semantics.
M6.7.zz.tenant.opt-out.cli.diff-timeline.system-only
...
not a list query). `--system-only` asserts BOTH events have
actor_id IS NULL (both system-authored); `--no-system` asserts
NEITHER event has actor_id IS NULL (both have UUID actor —
human or service-account). Mutually exclusive at CLI boundary
(both set returns exit 2 with `--system-only and --no-system
are mutually exclusive` error before any PG query matching
ADR-0209's history pattern). Composes with all existing
expectation flags (--kind, --kind-not, --actor-id,
--actor-id-not, --with-actor-names) without restriction.
Reuses the ActorPresenceFilter discriminated union from
ADR-0209 (same type, same CLI mutual-exclusivity, same JSON
envelope echo shape). DiffHistoryEntriesInput gains optional
actorPresence?: ActorPresenceFilter field. New expectation
check block positioned after the existing actorIdNot check
(actor-dimension group): system_only throws naming offending
side(s) with their actual actor_id value ("but A is '<uuid>'")
mirroring ADR-0203 positive --actor-id rendering — operators
need to see WHO authored the event if it wasn't supposed to
have a human actor; no_system throws naming offending side(s)
as <system> placeholder ("but A is <system>" / "both A and B
are <system>") mirroring ADR-0205 negative --actor-id-not
rendering — operators already know which side they expected to
be human, naming the sentinel suffices. Different rendering
reflects different information needs (positive needs to show
unexpected UUID; negative just names the side since <system>
is the excluded value). Check ordering: eventKind → eventKindNot
→ actorId → actorIdNot → actorPresence (kind-dimension first,
actor-dimension second with specific-actor checks before
presence checks). Real expectation-check use cases — system-
only audit pair verification (assert both events are scheduled
sweep events), human-only forensic comparison (assert neither
event is system before drawing responsibility conclusions),
compliance attestation gating (--no-system + --kind opt_out_set
asserts both opt_out_set + neither is system), migration
verification (--system-only + --kind retention_set asserts
both events are migration script signature). CLI threads
systemOnlyFlag + noSystemFlag via getBooleanFlag, mutual-
exclusivity check, translate to actorPresence single value,
thread to adapter, echo systemOnly + noSystem booleans in JSON
envelope (matching ADR-0209 history boolean-echo convention).
Composition with --actor-id / --actor-id-not — --system-only
+ --actor-id is contradictory at data layer (system has null
actor_id; --actor-id requires UUID; first check fires
surfacing actor-mismatch error — natural first-fail ordering),
--system-only + --actor-id-not is redundant but valid (system
events are NEITHER any specific UUID; both pass), --no-system
+ --actor-id is canonical "both must be human actor X" idiom,
--no-system + --actor-id-not is canonical "both must be human,
excluding X" idiom. Why expectation check not filter — diff-
history takes exactly two IDs nothing to filter; PG-side WHERE
filter would silently return zero rows on mismatch
indistinguishable from "IDs don't exist"; adapter throw with
clear message naming offending side(s) is loud and clear
matches ADR-0198/0203/0205/0208 precedent. Mismatch exits 1
(runtime path); CLI misuse mutual-exclusivity exits 2.
10 new adapter tests in a new "PostgresTraceRetention
.diffHistoryEntries actorPresence expectation check" describe
block: system_only accepts when both null actor_id, throws
naming A when A has UUID (with actual UUID in error), throws
naming B when B has UUID, throws naming both sides when
neither is system, no_system accepts when both have UUID
actor, throws when A is <system> (explicit <system> rendering),
throws when B is <system>, throws naming both when both are
<system>, omits check when actorPresence not set backward
compat, composes with --kind expectation check (both pass
when actor + kind expectations met). 9 new CLI tests in a
new "runRetention diff-history --system-only / --no-system"
describe block: exit 2 when both flags set with explicit
error, threads actorPresence='system_only' when --system-only
set, threads actorPresence='no_system' when --no-system set,
omits actorPresence when neither flag set backward compat,
composes with --actor-id-not + --no-system (both threaded
independently), adapter expectation error propagates as exit
1 with explicit error including actual UUID, JSON envelope
echoes systemOnly=true + noSystem=false when --system-only
set, JSON envelope echoes noSystem=true + systemOnly=false
when --no-system set, JSON envelope both false when neither
set. Test count: 8,966 → 8,985 (+19: adapter +10, CLI +9).
cli.ts helpText extended for retention diff-history usage
line with [--system-only | --no-system] notation indicating
mutual exclusivity + 4-line description added explaining
expectation-check semantic + mutual exclusivity. ADR-0212
documents the design + 10 rejected alternatives + 6 future
Qs (--system-only/--no-system on diff-timeline closes
ADR-0207 Q3 + ADR-0193 future Q, per-side asymmetric
expectation --system-only-a/--system-only-b pairs with
ADR-0203 Q1 + ADR-0205 Q2 family, short-circuit LEFT JOIN
when actorPresence='system_only' since system events have
no user row optimization, JSON envelope unification across
expectation-check flags tagged-union shape, apply
expectation-check semantic to retention prune --system-only,
--mixed-actors flag for "exactly one event is system other
is human" asymmetric uncommon defer). The retention diff-
history surface now has FIVE expectation-check flag families
forming 3×2 + 1 matrix — kind dimension (--kind + --kind-not)
+ specific actor_id dimension (--actor-id + --actor-id-not)
+ system actor presence dimension (--system-only +
--no-system from this milestone) + actor display
(--with-actor-names) — operators get full positive +
negative expectation gates across kind + specific-actor +
system-presence dimensions with consistent error-message
shape and uniform exit-code semantics. The --system-only /
--no-system family is now 2-of-3 surfaces shipped: retention
history (substrate-side filter from ADR-0209); retention
diff-history (expectation check from this milestone);
retention diff-timeline (substrate-side filter across all
3 paths from ADR-0207 Q3 + ADR-0193 future Q deferred). The
retention CLI now has 18 actions with --system-only /
--no-system support on both retention history (list-style
filter) and retention diff-history (cross-event expectation
check) — operators get system-vs-human ergonomics on the
two complementary audit-log surfaces with surface-
appropriate semantics.
M6.7.zz.tenant.opt-out.cli.history.actor-filter.multi
...
`retention history` using the multiFlags infrastructure from
ADR-0183. Mirror of ADR-0207's multi-value pattern on diff-
timeline; closes the within-family asymmetry where retention
history's --actor-id-not stayed single-value (ADR-0206) while
diff-timeline's went multi-value (ADR-0207). Real cohort-exclusion
use cases — service-account allowlist audit (exclude N known SAs
in one command), self-exclude reviewer cohort during incidents
(exclude my actor + on-call + SRE), compliance non-automation
filter (regulatory human review excluding 3-5 automation SAs),
cohort negative attestation (every mutation NOT made by platform-
admin cohort). Breaking adapter rename ListOptOutHistoryInput
.actorIdNot?: string → actorIdsNot?: ReadonlyArray<string>;
session-recent code from ADR-0206 with no external consumers
contained scope (matches ADR-0199 + ADR-0207 one-shot break
precedent — permanent two-field surface invites operator confusion
"which one wins when both set"; one-shot clean break beats
permanent two-field surface). SQL changes from single-value
`(h.actor_id IS NULL OR h.actor_id != $N)` to multi-value
`(h.actor_id IS NULL OR h.actor_id NOT IN ($N1, $N2, ...))`;
single-element NOT IN ($N) is PG-equivalent to = $N so no
observable behavior change for single-value callers (same query
plan). Empty array treated as filter-not-set matching ADR-0199/
0207 convention — operators may legitimately compute the list at
call site producing empty array (allowlist is empty); operator-
friendly. Why explicit IS NULL OR prefix preserved — PG NOT IN
returns NULL when actor_id IS NULL silently filtering system
events OUT (wrong intent — --actor-id-not means "exclude these
actors" which includes system events); explicit IS NULL OR prefix
restores correct inclusion same trap ADR-0206 documented now
extended to multi-value form. Why duplicates not deduplicated at
adapter — operators passing --actor-id-not <a> --actor-id-not <a>
get duplicate placeholders verbatim; PG handles fine via NOT IN
semantic substrate doesn't dedup matches ADR-0207 stance. CLI
runRetentionHistory reads via getMultiFlag("actor-id-not") instead
of getStringFlag; threads actorIdsNot to adapter; JSON envelope
renamed actorIdNot: string | null → actorIdsNot: string[] | null
matches ADR-0207's array-or-null canonical multi-value envelope
shape across the family. Within-surface asymmetry documented —
retention history's positive --actor-id stays single-value while
negative --actor-id-not now multi-value; documented as future Q
("widen --actor-id to multi-value on retention history would
close ADR-0186 future Q + restore within-surface symmetry");
deferred to keep milestone scope focused on closing ADR-0206 Q1
specifically. Asymmetry acceptable here because (1) positive +
negative semantically distinct dimensions — negative path benefits
more from multi-value since "exclude these N noise actors" is
more common than "include exactly these N specific actors", (2)
diff-timeline has BOTH multi-value (ADR-0199 + ADR-0207) for
operators wanting multi-value positive, (3) asymmetry operator-
visible via helpText + JSON envelope shape no silent behavior
change. cli.ts helpText updated from [--actor-id-not <uuid>] to
[--actor-id-not <uuid> ...] indicating repeatable + description
extended explaining repeatable + OR-semantic NOT IN + system-
events-included semantic. Adapter test block rewritten + expanded
from 8 to 11 tests under renamed "actorIdsNot filter (M6.7.zz
.tenant.opt-out.cli.history.actor-not + actor-not.multi)" describe
block — single-element NOT IN ($1) verified, multi-element NOT IN
($1, $2) with two placeholders verified, omits clause when not
set, empty array filter-not-set (NEW), includes system events
(IS NULL prefix preserved), composes with tenantId (multi-actor
+ tenant filter), composes with actorId (positive + negative
both threaded), composes with joinActor (LEFT JOIN + NOT IN both
present), composes with all filter dimensions (full param array
verified including multi-actor 7-param threading), returns rows
excluding listed actors (mock row + adapter result), duplicate
actorIdsNot values produce duplicate placeholders PG dedupes via
NOT IN semantic (NEW). Plus one cross-reference fix in system-
only describe block (`composes with actorIdNot + no_system` →
`composes with actorIdsNot + no_system`). CLI test block
rewritten + expanded from 6 to 8 tests under renamed
"runRetention history --actor-id-not (M6.7.zz.tenant.opt-out.cli
.history.actor-not + actor-not.multi)" describe block — threads
single-element array when set once, threads multi-element array
when --actor-id-not repeated (NEW), omits when not set backward
compat, composes with --actor-id positive + negative both
independently, composes with --tenant + --with-actor-names + multi
--actor-id-not, JSON envelope echoes single-element array,
JSON envelope echoes multi-element array (NEW), JSON envelope
actorIdsNot=null when not set. Test count: 8,955 → 8,960 (+5 net:
adapter +3, CLI +2). ADR-0210 documents the design + 10 rejected
alternatives + 7 future Qs (widen --actor-id to multi-value on
retention history closes ADR-0186 future Q + restores within-
surface symmetry, --actor-id-not @file.txt for very large lists,
CLI-side UUID validation per occurrence, substrate-side dedup of
duplicates, composite index on (actor_id, occurred_at) for scale,
--actor-name-not via meta.users JOIN, PG IN-list chunking for
1K+ excluded actors). The --actor-id-not family is now multi-
value uniformly across both filter surfaces — retention history
(this milestone) + retention diff-timeline (ADR-0207) — plus
single-value expectation check on the cross-event diff surface
(retention diff-history from ADR-0205); operators get cohort-
exclusion ergonomics with surface-appropriate semantics under
one consistent flag name. The retention CLI now has 18 actions
with multi-value --actor-id-not support on the two list-style
audit-log filter surfaces.
M6.7.zz.tenant.opt-out.cli.history.system-only
closes ADR-0186 + ADR-0206 + ADR-0205/0207 future Qs by adding
`--system-only` / `--no-system` boolean flag pair to `retention
history` for explicit null-actor filtering on the audit-log
surface. `--system-only` filters to events where actor_id IS NULL
(substrate-authored maintenance — prune sweeps, scheduled jobs,
expiry auto-lifts); `--no-system` filters to events where actor_id
IS NOT NULL (human / service-account authored deliberate
mutations). Operators wanting these queries previously jq-filtered
post-fetch which broke LIMIT correctness + cursor-pagination
correctness + index-usage parity — substrate-side WHERE is the
only way to get all three right (same reasoning as ADR-0186 +
ADR-0206 for value-based actor filtering). Mutually exclusive at
CLI boundary — both set returns exit 2 before any PG query with
"--system-only and --no-system are mutually exclusive" error.
Composes with every existing filter (--tenant + --table + --kind
+ --actor-id + --actor-id-not + --since + --until + --limit +
--after-id + --before-id + --range + --with-actor-names) without
restriction. Adapter changes — new exported ActorPresenceFilter
discriminated string union ("system_only" | "no_system");
ListOptOutHistoryInput gains optional actorPresence?:
ActorPresenceFilter field. Discriminated string union over two
booleans because (1) mutual exclusivity enforced at the type
level — impossible to express "both at once" in adapter input,
(2) adapter consumers see clean 3-state choice, (3) CLI translates
the two boolean flags into the single union value after the
mutual-exclusivity check. SQL adds one of two conditional WHERE
clauses (no parameters needed — IS NULL / IS NOT NULL are SQL
constructs not value comparisons): system_only adds h.actor_id
IS NULL; no_system adds h.actor_id IS NOT NULL. Position —
immediately after actorIdNot filter, before --since / --until
time-range clauses (matches existing actor-dimension grouping in
WHERE-clause assembly). CLI threads systemOnlyFlag + noSystemFlag
via getBooleanFlag, mutual-exclusivity check, translate to
actorPresence single value, thread to adapter, echo systemOnly +
noSystem booleans in JSON envelope (matching existing
withActorNames boolean-echo pattern from ADR-0185 — every CLI
flag has corresponding envelope field with same shape; mutual
exclusivity at CLI guarantees both are never true simultaneously).
Composition with existing actor filters — --system-only +
--actor-id is contradictory at data layer (system events have
actor_id IS NULL; --actor-id requires UUID; substrate returns
empty silently SQL natural outcome), --system-only + --actor-id-
not is redundant but valid (system events are NEITHER any specific
UUID by definition), --no-system + --actor-id is canonical "human
X's mutations" idiom (--no-system redundant since actor_id = $N
implies IS NOT NULL but harmless), --no-system + --actor-id-not
is canonical "human mutations excluding X" idiom (distinguished
from --actor-id-not alone which includes system events). Why
substrate-side not jq-side — LIMIT correctness, cursor-pagination
correctness, index usage parity (matches ADR-0186 + ADR-0206
reasoning). 8 new adapter tests in a new "PostgresTraceRetention
.listOptOutHistory actorPresence filter" describe block: adds
h.actor_id IS NULL when system_only, adds h.actor_id IS NOT NULL
when no_system, omits clause when not set, adds no params for
IS NULL/IS NOT NULL clauses (no placeholder pollution; LIMIT
param position correct), composes with tenantId + tableName
(system_only), composes with actorIdNot + no_system (redundant
but valid; both clauses present), composes with joinActor +
actorPresence (LEFT JOIN + IS NULL both present), returns rows
with null actor_id when system_only matches. 9 new CLI tests in
a new "runRetention history --system-only / --no-system" describe
block: exit 2 when both flags set with explicit error message,
threads actorPresence='system_only' when --system-only set,
threads actorPresence='no_system' when --no-system set, omits
actorPresence when neither set backward compat, composes with
--tenant + --system-only, composes with --actor-id-not +
--no-system (both threaded independently), JSON envelope echoes
systemOnly=true + noSystem=false when --system-only set, JSON
envelope echoes noSystem=true + systemOnly=false when --no-system
set, JSON envelope echoes both false when neither set. Test
count: 8,938 → 8,955 (+17: adapter +8, CLI +9). cli.ts helpText
extended for retention history usage line with [--system-only |
--no-system] notation indicating mutual exclusivity + 3-line
description added explaining IS NULL / IS NOT NULL semantic +
mutual exclusivity. ADR-0209 documents the design + 10 rejected
alternatives + 6 future Qs (--system-only/--no-system on diff-
history as expectation check pairing with ADR-0205 Q3,
--system-only/--no-system on diff-timeline across all 3 paths
pairing with ADR-0193 + ADR-0207 future Qs, partial index on
actor_id IS NULL for scale, short-circuit LEFT JOIN when
actorPresence='system_only' since system events have no user
row, per-side --system-only-a/--system-only-b on diff-history,
overloading --actor-id null sentinel rejected). The retention
history surface now has FIVE actor-related filter dimensions —
specific actor positive (--actor-id from ADR-0186) + specific
actor negative (--actor-id-not from ADR-0206) + system presence
positive (--system-only from this milestone) + system presence
negative (--no-system from this milestone) + actor display
(--with-actor-names from ADR-0185) — operators get first-class
system-vs-human + specific-actor inclusion/exclusion audit-log
workflows without LIMIT + pagination + index-usage correctness
problems of jq-filter workarounds. The `--system-only` /
`--no-system` family is now 1-of-3 surfaces: retention history
(substrate-side filter from this milestone); retention diff-
history (expectation check from ADR-0205 Q3 deferred); retention
diff-timeline (substrate-side filter across all 3 paths from
ADR-0207 Q3 + ADR-0193 future Q deferred). Subsequent milestones
close the remaining two surfaces mechanically following the
established per-surface precedent from --actor-id-not (ADR-0205
expectation check + ADR-0206 history filter + ADR-0207 diff-
timeline multi-value filter).
M6.7.zz.tenant.opt-out.cli.diff-history.kind-not
...
via --add-tenant + cross-table via --cross-table). Mirror of
ADR-0206's retention-history pattern on the diff-timeline
surface BUT multi-value to match ADR-0199's positive
multi-actor filter shape on diff-timeline — within-surface
consistency wins over cross-surface uniformity (retention
history single-value documented intentional divergence).
All three input types (DiffHistoryTimelineInput +
DiffHistoryTimelineNwayInput +
DiffHistoryTimelineCrossTableInput) gain actorIdsNot?:
ReadonlyArray<string>; SQL adds `(h.actor_id IS NULL OR
h.actor_id NOT IN ($N1, $N2, ...))` when set with non-
empty array — explicitly OR'd IS NULL so system events
INCLUDED (PG NOT IN returns NULL for null values silently
filtering systems out which is wrong intent); empty array
treated as filter-not-set matching ADR-0199 convention.
CLI uses getMultiFlag("actor-id-not") collecting repeated
flag occurrences, threads actorIdsNot to whichever of three
adapters dispatched, JSON envelope echoes actorIdsNot:
string[] | null across all three envelope shapes (pair-
wise no-discriminator, nway:true, crossTable:true)
positioned right after actorIds field for consistency.
NOT mutually exclusive with --actor-id — both clauses fire
independently at SQL layer; contradictory --actor-id X
--actor-id-not X returns empty silently (SQL natural
outcome). Why parallel implementation across three paths —
matches ADR-0193/0194/0199/0200/0202 pattern; mechanical
+ structurally identical addition mirrors how every other
diff-timeline filter exists three times. Use cases unblocked
— cohort drift detection excluding noise actors (5-tenant
cohort × exclude migration + sweep SAs), cross-table audit
excluding automation, self-exclude reviewer audit during
incident window, compose positive + negative for fine-
grained filter with overlapping actor cohorts. Rejected
alternatives — CLI-side jq filter (breaks LIMIT
correctness), single-value to match ADR-0206 retention
history (would diverge from diff-timeline's existing
multi-value actorIds; within-surface consistency wins),
NOT IN alone without IS NULL OR (silently filters system
events), IS DISTINCT FROM per-value (doesn't compose with
IN list), mutually exclusive with --actor-id at CLI
boundary (would block valid intentional overlap patterns
adapter natural empty result cleaner), substrate-side
overlap rejection (SQL handles correctly), pipe-separated
single flag (operators with shell pipes hit edge cases),
three separate flags per dispatch path (uniform flag
matches established pattern). Drawbacks — multi-value
asymmetry with ADR-0206 retention history (documented
intentional follows ADR-0186 vs ADR-0193/0199 divergence
future widening Q exists), includes system events by
design, contradictory composition returns empty silently
(acceptable obvious), no CLI-side UUID validation, three-
path parallel implementation (structural symmetry mirrors
established pattern), no pipe-separated single flag form
(repeated flag is canonical). 9 new adapter tests in a
new "PostgresTraceRetention diff-timeline --actor-id-not
filter" describe block — pair-wise single excluded actor
adds NOT IN ($N) WHERE clause verified, pair-wise multi-
excluded actors adds NOT IN ($N1, $N2) with multiple
placeholders, pair-wise omits clause when not set, pair-
wise treats empty array as filter-not-set, N-way single
excluded actor positioned after tenant IN list, N-way
multi-excluded actors, cross-table single excluded actor
positioned after table IN list, pair-wise composes with
actorIds (positive + negative both clauses present), pair-
wise composes with joinActor + eventKinds + actorIdsNot
(LEFT JOIN + NOT IN + IN all 3 present). 10 new CLI tests
in a new "runRetention diff-timeline --actor-id-not"
describe block — pair-wise threads single actorIdsNot
when --actor-id-not set, pair-wise threads multi
actorIdsNot when --actor-id-not repeated, pair-wise omits
when NOT set backward compat, N-way threads actorIdsNot
alongside --add-tenant, cross-table threads alongside
--cross-table, composes with --actor-id positive +
negative both threaded, pair-wise JSON envelope echoes
actorIdsNot when set, N-way JSON envelope echoes, cross-
table JSON envelope echoes, pair-wise JSON
actorIdsNot=null when NOT set. Test count: 8,904 → 8,923
(+19: adapter +9, CLI +10). cli.ts helpText extended on
both diff-timeline usage lines with [--actor-id-not
<uuid>] flag + 2-line description explaining excludes-
listed-actors + system-events-included + repeatable
semantic. ADR-0207 documents the design + 8 rejected
alternatives + 6 future Qs (--actor-id-not on diff-
history already covered as expectation check from ADR-
0205, widen retention history --actor-id-not to multi-
value pairs with ADR-0206 Q1, --system-only + --no-
system explicit null actor matching, --kind-not exclusion
companion symmetric flag on diff-timeline, composite
index on (actor_id, occurred_at) for scale, --actor-
name-not filter via meta.users JOIN). The retention CLI
now has 18 actions with --actor-id-not support spanning
THREE retention audit-log surfaces — retention history
(substrate-side filter from ADR-0206), retention diff-
history (expectation check from ADR-0205), retention
diff-timeline (substrate-side multi-value filter across
all 3 paths from this milestone) — operators get full
positive + negative actor expectations across all
audit-log surfaces with surface-appropriate semantics
(single-value vs multi-value) under one consistent flag
name.
M6.7.zz.tenant.opt-out.cli.history.actor-not
closes ADR-0186 Q1 + ADR-0205 Q6 by adding `--actor-id-not
<uuid>` substrate-side WHERE NOT filter to `retention history`
— inverse of ADR-0186's positive --actor-id filter on the list-
style audit-log surface. Different semantic from ADR-0205's
exclusion expectation check on diff-history (which throws on
match in a 2-fixed-event compare) — this is a list-style query
filter excluding rows where actor_id matches. Use cases —
service-account-free human audit (--actor-id-not <ci-sa>
returns human + system events excluding SA), suspended-user
activity audit (everyone else's mutations during period),
self-exclude review (--actor-id-not $MY_ACTOR_ID for
operator's own activity exclusion), compliance non-automation
filter (regulatory "human review" event lists excluding
migration/sweep SAs). ListOptOutHistoryInput gains optional
actorIdNot?: string field; when set SQL adds `(h.actor_id IS
NULL OR h.actor_id != $N)` WHERE clause — explicitly OR'd IS
NULL so system events are INCLUDED (matches operator
expectation "everything not Alice" includes system events;
PG's actor_id != $N returns NULL for null values silently
filtering systems out which is wrong). Substrate-side not
jq-side filter for LIMIT correctness + future cursor-pagination
correctness + index usage parity (matches ADR-0186 reasoning).
Filter position right after positive actorId filter in WHERE-
clause assembly. CLI threads --actor-id-not via getStringFlag,
passes actorIdNot to adapter, echoes actorIdNot: string | null
in JSON envelope right after actorId field for consistency
with WHERE-clause ordering. NOT mutually exclusive with
--actor-id — both clauses fire independently; --actor-id alice
--actor-id-not bob returns Alice's events (Alice != Bob so
clause passes); contradictory --actor-id X --actor-id-not X
returns empty silently (no error operators see empty result
naturally — explicit error path adds complexity for unlikely
misuse). No CLI-side UUID validation matches established
pattern. Rejected alternatives — CLI-side jq filter (breaks
LIMIT correctness), use `actor_id != $N` alone (silently
filters system events losing operator intent), IS DISTINCT
FROM single-operator alternative (functionally equivalent
explicit form clearer for SQL readers), mutually exclusive
with --actor-id at CLI boundary (would block valid
composition), multi-value via repeated flag from day one
(defer single-value common case), inverse flag named
--exclude-actor-id (verbose --actor-id-not matches ADR-0205
naming), substrate-side rejection of contradictory
combinations (SQL empty result is natural outcome),
filter at result-mapping layer (same LIMIT correctness
problem as jq). Drawbacks — single-value exclusion only
(multi-value defer), includes system events by design
(operators wanting human-only need separate flag defer),
contradictory composition returns empty silently (acceptable
since contradiction is logically obvious), no CLI-side UUID
validation, no --actor-id-not on diff-timeline yet (different
surface defer), no --system-only / --no-system flag (operators
jq-filter defer). 8 new adapter tests in a new
"PostgresTraceRetention.listOptOutHistory actorIdNot filter"
describe block — adds (h.actor_id IS NULL OR h.actor_id != $N)
WHERE clause when set verified via SQL substring + params,
omits actorIdNot WHERE clause when not set, includes system
events (null actor_id) when filtering verified via SQL
substring, composes with tenantId (both filters present),
composes with actorId both threaded though contradictory at
SQL level, composes with joinActor + actorIdNot (LEFT JOIN +
WHERE clause both present), composes with all filter dimensions
(7-param threading verified), returns rows excluding the actor
when adapter returns filtered results. 6 new CLI tests in a
new "runRetention history --actor-id-not" describe block —
threads actorIdNot when set, omits when NOT set backward compat,
composes with --actor-id both threaded independently, composes
with --tenant + --with-actor-names + --actor-id-not (3-flag
combination), JSON envelope echoes actorIdNot when set, JSON
actorIdNot=null when NOT set. Test count: 8,890 → 8,904 (+14:
adapter +8, CLI +6). cli.ts helpText extended for retention
history usage line with [--actor-id-not <uuid>] flag + 2-line
description explaining excludes-that-actor + system-events-
included semantic. ADR-0206 documents the design + 8 rejected
alternatives + 6 future Qs (--actor-id-not multi-value via
repeated flag using multiFlags from ADR-0183, --system-only +
--no-system explicit null actor_id matching, --actor-id-not
filter on diff-timeline all three paths, --kind-not filter
companion symmetric flag, composite index on (actor_id,
occurred_at) for scale, --actor-name-not filter via
meta.users JOIN). The retention CLI now has 18 actions with
--actor-id-not support spanning TWO retention audit-log
surfaces — retention diff-history (expectation check from
ADR-0205) and retention history (substrate-side filter from
this milestone) — operators get full positive + negative
actor expectations across both cross-event and list-style
query surfaces with surface-appropriate semantics under one
consistent flag name.
M6.7.zz.tenant.opt-out.cli.diff-history.actor-not
...
Adapter `DiffHistoryEntriesInput` gains `joinActor?: boolean`;
when set the SELECT widens to include u.display_name + u.email
via LEFT JOIN on actor_id; result type
`DiffHistoryEntriesResult` gains required `actorIdA` +
`actorIdB` fields (always populated since ADR-0203 added the
actor_id column to the SELECT) and optional `actorDisplayNameA/B`
+ `actorEmailA/B` populated only when joinActor=true. SQL is
aliased h. consistently (with or without JOIN) matching ADR-0185
+ ADR-0190 pattern. CLI runRetentionDiffHistory reads
--with-actor-names via getBooleanFlag, threads joinActor to
adapter, passes opts.withActorNames into formatHistoryDiff which
appends `by <actor>` suffix on each event line using the
generalized formatActor helper from ADR-0190 (display_name
(uuid) → email (uuid) → raw uuid → <system> for null actor_id —
same rendering rules as ADR-0185/0190). Composes with --kind
(ADR-0198) + --actor-id (ADR-0203) without restriction. JSON
envelope gains withActorNames boolean discriminator + result
conditionally carries actorDisplayName/Email fields when set.
Minor breaking change in DiffHistoryEntriesResult — actorIdA +
actorIdB are now required where they were absent in ADR-0173;
data has always been available (actor_id column existed since
ADR-0170, SELECT widened in ADR-0203); making them required
reflects reality and gives operators always-on access for the
no-JOIN path; contained-scope no production consumer. Use cases
unblocked — one-command "who made each change" diff audit,
compliance review with named actor attribution, compose with
--actor-id expectation check + display for forensic discipline,
incident post-mortems with named actor timelines. Rejected
alternatives — always-on JOIN no flag (adds query cost defeats
opt-in pattern matches ADR-0185/0190), new action
diff-history-with-actors (adds CLI surface flag-on-existing
matches established pattern), INNER JOIN (drops orphan-FK +
null-actor events loses audit context), display_name only
without UUID (strips forensic disambiguation matches ADR-0185
stance), separate --actor-names-format flag (overkill formatActor
falls back naturally), conditional actorIdA/B only when JOIN set
(splits path between always-available data and conditional;
making required cleaner), required null display fields when
joinActor=false (changes JSON shape backward-compat broken),
cache user lookups across paginated calls (substrate stateless
operators wrap at their layer bounded JOIN cost makes
unnecessary at 2 rows per call). Drawbacks —
DiffHistoryEntriesResult minor breaking change (contained scope
new actorId* fields reflect always-available data), conditional
display fields on result (matches ADR-0185 OptOutHistoryEntry +
ADR-0190 TimelineEntry convention consumers branch via
result.actorDisplayNameA !== undefined), LEFT JOIN cost per
query when flag set (meta.users.id PK index-only + LEFT JOIN
doesn't expand rows + bounded by 2 rows negligible at typical
scale), cross-schema dependency on meta.users when joinActor
=true (substrate ships both tables together not regression), no
human-format change to field diffs (only metadata header gains
actor suffix; per-field actor attribution different surface
defer), no multi-tenant scoping on meta.users (same caveat as
ADR-0185/0190 cross-tenant actor renders with canonical
display_name matches operator intent "who did this?"). 8 new
adapter tests in trace-retention.test.ts in a new
"PostgresTraceRetention.diffHistoryEntries --with-actor-names"
describe block — omits LEFT JOIN when joinActor not set
backward compat verified, emits LEFT JOIN meta.users when
joinActor=true with display_name + email columns in SELECT,
returns actor info for both events when joinActor=true + users
exist (Alice + Bob both populated), returns null
actorDisplayName + actorEmail when actor has no user row
(orphan FK case for B only), returns null for system actor
(null actor_id for A), omits actor display fields when
joinActor is false (TypeScript undefined), always populates
actorIdA + actorIdB even without joinActor (from existing
actor_id column), composes with --actor-id expectation check
(joinActor + actorId both threaded). 9 new CLI tests in
retention.test.ts in a new "runRetention diff-history
--with-actor-names" describe block — threads joinActor=true to
adapter when --with-actor-names is set, omits joinActor when
flag NOT set backward compat, human-format renders 'by Alice
Smith (uuid)' suffix when names populated for each event,
renders <system> for null actor_id, falls back to email when
display_name is null, omits 'by ...' suffix when flag NOT set,
JSON envelope echoes withActorNames=true + actor fields when
set, JSON withActorNames=false when NOT set, composes with
--actor-id + --kind (all three flags threaded). Test count:
8,859 → 8,876 (+17: adapter +8, CLI +9). cli.ts helpText
extended for retention diff-history usage line with
[--with-actor-names] flag + 3-line description explaining LEFT
JOIN meta.users semantic + fallback rendering. ADR-0204
documents the design + 8 rejected alternatives + 6 future Qs
(--actor-name-equals filter, show user status alongside name,
display tenant_membership role would need additional JOIN,
per-field actor attribution different surface, --actor-id-not
exclusion to compose with display, apply --with-actor-names
pattern uniformly to other diff-* surfaces). The retention CLI
now has 18 actions with --with-actor-names support uniform
across THREE retention audit-log surfaces — retention history
(ADR-0185), retention diff-timeline (ADR-0190 across all three
paths), retention diff-history (this milestone) — operators get
human-readable actor attribution uniformly across single-tenant
audit-log + cross-tenant timeline diff + cross-event policy
diff workflows.
M6.7.zz.tenant.opt-out.cli.diff-history.actor-filter
closes ADR-0198 Q3 by threading `--actor-id <uuid>` through
`retention diff-history` as an expectation check (not a filter,
since diff-history takes exactly two IDs not a list query) —
mirror of ADR-0198's `--kind` pattern on the actor dimension.
Adapter SELECT widened to include `actor_id` column; result row
type gains `actor_id: string | null` field; new actorId
expectation check fires after the existing eventKind check —
validates `entryA.actor_id === input.actorId` AND
`entryB.actor_id === input.actorId`, throwing with explicit
'expected both events to have actor_id <X> but A is <Y> and B
is <Z>' on mismatch naming the offending side(s); null actor_id
renders as `<system>` matching ADR-0185 retention history
convention — operators reading mismatch messages can't confuse
the system-actor sentinel with a literal "null" string. CLI
threads `--actor-id <uuid>` flag via getStringFlag (no UUID
validation matching ADR-0175/0186/0193 deferred decision), passes
actorId to adapter call, echoes `actorId: string | null` in JSON
envelope matching ADR-0193 convention. Composes with `--kind`
from ADR-0198 — both expectations applied independently, either
mismatch throws. Why expectation-check not filter — diff-history
takes exactly two IDs not a list, flag semantic shifts from
filter to expectation check matching ADR-0198's pattern; PG-side
WHERE filter would silently return zero rows on mismatch
indistinguishable from "IDs don't exist", adapter throw with
clear message naming the offending side is loud and clear.
Mismatch exits 1 (runtime path matching cross-tenant + cross-
table + eventKind mismatch error propagation), invalid input
exits 2 (CLI misuse). Use cases unblocked — compliance script
asserting actor match ('--actor-id <alice>' diff fails when
accidentally comparing across actors), forensic analysis gated
on actor (compose with --kind for fine-grained expectation
checks like '--actor-id <suspect> --kind opt_out_set'), CI gate
verifying service-account migration ('--actor-id <migration-sa>
--kind retention_set' exits 1 if migration ran as wrong actor),
detect inadvertent cross-actor comparison (operator forensic
discipline — fails loudly when picking wrong ID for either
side). Rejected alternatives — --actor-id as filter limiting
fieldDiffs (doesn't match per-event semantic of actor_id which
is row-level not field-level), make mismatch warn not throw
(silent gates lose safety property), two flags --actor-id-a +
--actor-id-b for per-side expectations (overkill for v1 defer),
PG-side WHERE clause validation (would silently return zero
rows operators can't distinguish from missing IDs), type adapter
input as string | null to allow null filtering (couples shape
to null-actor use case operators jq-filter), JSON envelope field
named expectedActorId (verbose; actorId matches ADR-0193
convention), render mismatch as fieldDiff (actor_id isn't field
in next_state JSONB throwing is loud), use actorId IN filter
semantic (would silently return zero rows expectation-check
throw clearer). Drawbacks — semantic shift from filter to
expectation check (mitigated by explicit error message matching
ADR-0198 caveat), no null-actor sentinel for input (operators
wrap with jq matches ADR-0186 null-actor stance), no CLI-side
UUID validation matches deferred decision, single-actor only no
multi-value tuple (different shape than multi-actor OR-filter
from ADR-0199 since expectation has "all must be one of N"
semantic vs filter's "any match"), no per-event expectation
(operators with different per-side expectations jq-filter JSON
defer), two new adapter errors with similar shape (same-tenant/
same-table/eventKind already throw with explicit error actorId
joins family acceptable structural consistency). 7 new adapter
tests in trace-retention.test.ts in a new "PostgresTraceRetention
.diffHistoryEntries --actor-id expectation check" describe block
— accepts when both events have expected actor_id, throws when
event A's actor mismatches with explicit error naming side A,
throws when event B's actor mismatches naming side B, throws
naming both sides when neither matches (including <system>
rendering for null actor_id), renders <system> for null actor_id
in mismatch message, omits the check when actorId not set
backward compat verified, composes with eventKind check (both
pass). 6 new CLI tests in retention.test.ts in a new
"runRetention diff-history --actor-id" describe block — threads
actorId to adapter when --actor-id set, omits actorId when
--actor-id NOT set backward compat, composes with --kind
threading both eventKind and actorId, adapter mismatch error
propagates exit 1 with explicit error including expected +
actual actors, JSON envelope echoes actorId field when set,
JSON envelope actorId=null when NOT set. Test count: 8,846 →
8,859 (+13: adapter +7, CLI +6). cli.ts helpText extended for
retention diff-history usage line with [--actor-id <uuid>] flag
+ 2-line description explaining expectation-check semantic +
mismatch-exits-1 contract + <system> rendering for null
actor_id. ADR-0203 documents the design + 8 rejected
alternatives + 6 future Qs (per-side --actor-id-a + --actor-id-b
for asymmetric expectations, --actor-id <a>|<b>|<c> multi-value
expectation, --with-actor-names on diff-history LEFT JOIN
meta.users for the two events' actors, --actor-name-equals
filter via meta.users lookup pairing with ADR-0186 Q3, CLI-side
UUID validation matching ADR-0175 deferred decision,
--system-only flag for both events being system-authored).
The retention CLI now has 18 actions with --actor-id support
uniform across THREE retention diff family members — retention
history (filter from ADR-0186), retention diff-timeline (filter
from ADR-0193/0199 across all three paths), retention diff-
history (expectation check from this milestone) — semantics
adapt to the surface (list-style filter vs cross-event
expectation check) but the flag name + UUID input + helpText
behavioral note are consistent across the family.
M6.7.zz.tenant.opt-out.cli.diff-timeline.range
closes ADR-0197 Q1 + ADR-0201 Q1 by mirroring ADR-0201's `--range
<after-id>..<before-id>` window-cursor flag onto all three
`retention diff-timeline` dispatch paths (pair-wise + N-way via
--add-tenant + cross-table via --cross-table). Pure CLI delivery
— no adapter changes since all three diff-timeline adapter inputs
already accept both afterId + beforeId simultaneously (substrate-
level range-cursor preserved across ADR-0195 + ADR-0197); --range
is the explicit operator gesture to opt into the range semantic
that the CLI was previously gating. Same parse + validation +
mutual-exclusivity pattern as ADR-0201 — git-like `..` separator,
exit 2 on missing separator + empty halves + > 2 parts, exit 2
when combined with bare --after-id or --before-id (since --range
sets both), bare-flag mutual-exclusivity error updated to suggest
--range. CLI dispatcher reads rangeFlag once + computes effective
afterId/beforeId from either rangeFlag halves or bare flags;
threaded to whichever of three adapters dispatched. JSON envelope
gains range: string | null field on all three envelope shapes
(pair-wise no-discriminator, nway:true, crossTable:true) echoing
operator's literal input + afterId + beforeId fields ALSO
populate when --range set for jq scripts reading either path.
Why parallel implementation across all three paths — the three
paths already use shared cursor + JSON envelope code; adding
range to each is mechanical and structurally identical (mirrors
how every other diff-timeline filter from --actor-id through
--kind through --after-id/--before-id already exists three
times); one milestone covers all three because the lift is
identical and the operator-facing surface stays consistent.
Window semantics depend on ASC ordering — diff-timeline orders
ASC (chronological) so --range <a>..<b> returns events newer
than a AND older than b (operators reversing cursor order get
empty results correctly); INVERSE direction from retention
history's DESC range semantic documented in helpText. Use cases
unblocked — anchor-based bidirectional cohort audit (5-tenant
cohort × incident-trigger..post-incident-cursor window), cross-
table window scoped to migration period (one tenant × 3 tables
× migration-start..migration-end), per-actor per-window forensic
narrowing composing multi-actor × multi-kind × range cursor in
one command, CI gate "no mutations in protected window" via jq
on result.entries length. Rejected alternatives — single-path
delivery (would force N-way + cross-table operators to bare
flags only; symmetric coverage matches established pattern),
different separator on diff-timeline vs history (confuses
operators learning both surfaces; same `..` is consistent),
reverse range semantic on diff-timeline (treat first half as
upper bound; would diverge from ADR-0201 convention where first
half is always --after-id cursor), two flags --from + --to
(verbose loses single-flag ergonomic), adapter-side accept
range parameter directly (pushes parsing into substrate where
it doesn't belong CLI is right boundary), no mutual-exclusivity
check (could mask operator confusion explicit error helps).
Drawbacks — parallel implementation across three paths
(structural symmetry mirrors established pattern), no adapter
changes pure CLI ergonomics, `..` separator strictly 2 dots
not 3 (matches ADR-0201), no CLI-side UUID validation matches
deferred decision, range envelope echoes raw operator input
matches ADR-0201, window semantics depend on ASC ordering vs
DESC on retention history (documented in helpText). 14 new CLI
tests in a new "runRetention diff-timeline --range" describe
block — pair-wise parses --range and threads both cursors, N-way
threads alongside --add-tenant, cross-table threads alongside
--cross-table (3 path coverage), exits 2 on missing separator
+ empty after-id half + empty before-id half + with --after-id
+ with --before-id (5 exit-2 cases), bare --after-id + --before-
id without --range error message points at --range, JSON
envelope echoes range + afterId + beforeId on pair-wise + JSON
range=null when not set, JSON envelope echoes range on N-way
path + cross-table path (3 envelope-shape tests), composes with
--actor-id + --kind + --with-actor-names + --since verified
multi-actor + multi-kind + window cursor + actor JOIN in one
command. Test count: 8,832 → 8,846 (+14). cli.ts helpText
extended on both diff-timeline usage lines with [--range <after-
id>..<before-id>] + descriptions explaining single-flag window-
cursor semantic + mutual exclusivity rules. ADR-0202 documents
the design + 6 rejected alternatives + 6 future Qs (--range on
diff-history likely not needed since diff-history takes 2
positional IDs not a list, strict-inclusive >=/<= cursor
semantic same as ADR-0195 Q6 + ADR-0201 Q2, range bounds
visualization in human output, --range @file.txt batch, parsed-
object envelope form, composite index for scale). The retention
CLI now has 18 actions with window-cursor pagination uniform
across BOTH audit-log surfaces — retention history --range from
ADR-0201 (DESC) and all three retention diff-timeline paths
--range from this milestone (ASC). Bidirectional pagination
(--after-id / --before-id) + window cursor (--range) +
multi-actor + multi-kind + cohort comparison (N-way + cross-
table) all compose across the diff-timeline surface in a single
command.
M6.7.zz.tenant.opt-out.cli.history.range
...
shape on the kind filter dimension. Single value scopes to one
event_kind; multiple values build OR-semantic IN clause ("every
opt_out_set OR opt_out_cleared event"). Zero values preserves
no-filter behavior. Breaking adapter type rename — all three
diff-timeline input types rename eventKind?: OptOutHistoryEventKind
→ eventKinds?: ReadonlyArray<OptOutHistoryEventKind>; contained
scope matches ADR-0199's one-shot break pattern. CLI uses
getMultiFlag("kind") + validates each occurrence individually
via isOptOutHistoryEventKind (exit 2 with the offending value
named on any invalid); single-element validates → IN ($N),
multi-element validates → IN ($N1, $N2, ...). Retention history
+ retention diff-history keep eventKind: single intentionally
— history is list-style with single-kind filter common,
diff-history is per-event-pair expectation check (different
semantic). JSON envelope renames kind: OptOutHistoryEventKind |
null → kinds: OptOutHistoryEventKind[] | null across all three
diff-timeline envelope shapes (pair-wise no-discriminator,
nway:true, crossTable:true). Use cases unblocked — opt-out
lifecycle audit (--kind opt_out_set --kind opt_out_cleared
across cohort), per-actor per-kinds forensics (Alice OR Bob ×
opt_out_set OR opt_out_cleared), positive exclusion of noise
kinds (3-of-4 kinds in one filter), compliance attestation
across cohort × kinds. Rejected alternatives — keep eventKind
+ add eventKinds alongside (two-field surface; matches ADR-0199
decision), --kind a|b|c pipe-separated (shell-quoting concern),
--kinds plural alias (adds CLI surface), validate batch all at
once (per-value validation gives crisper error), reject
duplicates at CLI (PG handles fine), PG enum-array
`= ANY($N::text[])` (inconsistent with substrate's positional-
placeholder pattern). Drawbacks — breaking type + envelope
rename (contained scope same one-shot pattern as ADR-0199),
retention history eventKind stays single divergence
(intentional documented), retention diff-history eventKind
stays single different semantic (expectation check ADR-0198
Q2 covers multi-kind), OR-semantic only no NOT filter (defer),
no duplicate dedup (PG handles). Adapter test block rewritten +
expanded from 7 to 11 tests covering single + multi-kind across
all three dispatch paths + empty-array edge case + composition
with actorIds. CLI test block rewritten + expanded from 9 to
12 tests covering invalid-on-any-occurrence exit 2 + single +
multi-kind threading + JSON envelope echoes on all three paths
+ composition with multi-actor. Test count: 8,815 → 8,822 (+7
net: adapter +4, CLI +3). cli.ts helpText extended for both
diff-timeline usage lines noting --actor-id + --kind both
repeatable with OR-semantic filter behavior. ADR-0200 documents
the design + 6 rejected alternatives + 5 future Qs (widen
retention history's eventKind to eventKinds for consistency,
--kind-not exclusion, multi-kind expectation on diff-history
pairing with ADR-0198 Q2, composite index on (event_kind,
occurred_at) for scale, --kind @file.txt overkill since only 4
valid values exist). The retention CLI now has 18 actions with
multi-value OR filters uniform across both actor + kind
dimensions on the diff-timeline surface — operators get cohort-
based forensics composing (N tenants OR M actors) × (P kinds)
× (time window) × (cursor pagination) × (cross-table or N-way
or pair-wise dispatch) in a single command across the three
diff-timeline paths.
M6.7.zz.tenant.opt-out.cli.diff-timeline.multi-actor
closes ADR-0193 Q1 by making `--actor-id <uuid>` repeatable on
`retention diff-timeline` across all three dispatch paths (pair-
wise + N-way via --add-tenant + cross-table via --cross-table)
via the multiFlags infrastructure from ADR-0183/0188. Single
value scopes to one actor's events; multiple values build an
OR-semantic IN-clause filter ("Alice OR Bob's mutations"). Zero
values (flag omitted) preserves no-actor-filter behavior. Breaking
adapter type change — all three diff-timeline input types rename
actorId?: string → actorIds?: ReadonlyArray<string>; contained
scope (session-recent code from ADR-0193, no external consumers,
no production surface affected); one-shot clean break beats
permanent two-field surface; same shape as ADR-0183's tenantIds
+ ADR-0188's tableNames consistent across N-way collection
inputs. SQL builds positional placeholders for IN clause: single-
element array yields `h.actor_id IN ($N)` equivalent to old
`h.actor_id = $N` (PG handles both identically), multi-element
yields `h.actor_id IN ($N1, $N2, ...)`. Empty array treated as
filter-not-set (no WHERE clause added). CLI uses getMultiFlag
("actor-id") returning string[]; threads to actorIds when
length > 0. JSON envelope renames actorId: string | null →
actorIds: string[] | null across all three envelope shapes
(pair-wise no-discriminator, nway:true, crossTable:true). The
retention history surface keeps actorId: string (single value)
intentionally — divergence with diff-timeline's actorIds
documented as future Q; separate milestone could widen if
measured demand emerges (operators currently chain two
commands for the rare multi-actor on history case). Use cases
unblocked — two-actor cohort forensics ("Alice OR Bob's
mutations across both tenants on workflow_traces during audit
window"), service-account allowlist audit (3 SAs + cross-cohort
+ --kind retention_set), per-team mutation audit on shared
table (3 team leads × cross-table), compliance reviewer audit
set (5 auditors × 5-tenant regulated cohort × full Q1 export).
Rejected alternatives — keep actorId + add actorIds alongside
(two-field surface complicated by merge/reject logic; one-shot
rename cleaner), actorId as string | ReadonlyArray<string> union
(forces every consumer to discriminate), CLI builds OR via jq
post-filter (breaks LIMIT correctness + cursor pagination same
trap ADR-0193 closed), PG `= ANY($N::uuid[])` instead of IN
positional placeholders (cleaner SQL but inconsistent with
substrate's positional-placeholder pattern throughout), widen
retention history actorId in same milestone (scope creep defer),
--actor-id comma-separated values (operators with shell vars
could trip on embedded commas), --actors plural alias (adds
surface for one concept), reject duplicate values at CLI
(substrate doesn't need dedup; PG handles duplicates fine).
Drawbacks — breaking field rename across three adapter input
types + JSON envelope (contained scope no external consumers
documented one-shot break), retention history actorId stays
single divergence with diff-timeline (intentional documented
future-Q if demand emerges), OR semantic only (substrate has
one actor per row AND semantic would need different data
model out of scope), no actor deduplication (PG handles
duplicates fine substrate doesn't need to dedup), no null-actor
sentinel (operators jq-filter for system events same as
ADR-0193 deferred decision), no CLI-side UUID validation
(matches ADR-0193 deferred). Adapter tests in trace-retention.
test.ts — the actor-filter describe block was rewritten with
breaking-rename in mind, expanded from 7 to 11 tests: pair-
wise IN($N) for single, pair-wise multi-actor IN($N1, $N2)
verifying OR semantic, pair-wise omits when not set, pair-wise
omits when empty array (treats as filter-not-set), N-way IN
single/multi positioned after tenant IN list, cross-table IN
single/multi positioned after table IN list, pair-wise composes
actorIds with --since + --until + --limit + joinActor, N-way
omits, cross-table omits — verifies the full SQL substring +
params expectations under the new field name. CLI tests in
retention.test.ts — rewrote the --actor-id describe block
expanded from 8 to 11 tests: pair-wise threads single as
[ACTOR_A], multi-actor threads as [ACTOR_A, ACTOR_B] via two
--actor-id flag occurrences, pair-wise omits actorIds when not
set backward compat, N-way + multi-actor threads alongside
--add-tenant, cross-table + multi-actor alongside --cross-
table, composes with --with-actor-names + --since + --limit,
JSON envelope echoes actorIds: [ACTOR_A] single, actorIds:
[ACTOR_A, ACTOR_B] multi, actorIds=null when flag NOT set,
JSON envelope on N-way + cross-table paths echoes actorIds
array. Test count: 8,809 → 8,815 (+6 net: adapter +4, CLI +2).
cli.ts helpText extended on both diff-timeline usage lines —
--actor-id flag is now described as repeatable with OR-semantic
multi-actor filter wording. ADR-0199 documents the design + 8
rejected alternatives + 7 future Qs (widen retention history's
actorId to actorIds for consistency, --actor-id-not exclusion,
--actor-name-equals repeated via meta.users JOIN, multi-actor
+ --system-only mode for null actor_id, composite index on
(actor_id, occurred_at), --actor-id @file.txt reading from
file, range cursor + multi-actor composition noted as already-
correct no-action-needed). The retention CLI now has 18 actions
with multi-actor filter on the diff-timeline surface — operators
get OR-semantic actor cohorts across pair-wise + N-way + cross-
table dispatch paths uniformly. Retention history's actor
filter stays single value intentionally; divergence documented.
M6.7.zz.tenant.opt-out.cli.diff-history.kind-filter
...
tenant via --add-tenant + cross-table via --cross-table. Brings
diff-timeline to bidirectional-pagination parity with retention
history (ADR-0196 just shipped --before-id there). Cursor SQL
flips operator from ADR-0195's `>` to `<` since diff-timeline
orders ASC (oldest first) and walking backward in time toward
older entries means going UP the list — `(h.occurred_at, h.id) <
(cursor)` returns earlier-timestamp entries. Mirror table: for
retention history (DESC), --after-id uses `<` (walk-down = older)
and --before-id uses `>` (walk-up = newer); for diff-timeline
(ASC), --after-id uses `>` (walk-down = newer) and --before-id
uses `<` (walk-up = older). Operator semantics consistent across
surfaces: --after-id always means "next chronologically-forward
page" and --before-id always means "previous chronologically-
backward page" regardless of underlying SQL operator. All three
diff-timeline input types gain optional beforeId?: string field;
all three adapter methods add identical conditional WHERE clause
when set positioned right after the afterId block before
params.push(limit). Adapter accepts both afterId + beforeId
simultaneously (substrate-level range-cursor semantic preserved
across all three diff-timeline paths matching ADR-0196 history
pattern) while CLI enforces mutual exclusivity at boundary —
both flags set returns exit 2 with clear error. CLI dispatcher
in runRetentionDiffTimeline reads both flags via getStringFlag,
checks mutual exclusivity BEFORE reading positionals (matches
ADR-0192's flag-validation-first pattern), threads beforeId
through to whichever of the three adapter methods is selected.
nextBeforeId computed at CLI layer for all three paths as the
FIRST entry's id when entries.length === limit (boundary for
going-older in ASC ordering — first entry is oldest); when
length < limit no more older entries → null. JSON envelope
gains beforeId: string | null + nextBeforeId: string | null
fields across all three envelope shapes (pair-wise no-
discriminator, nway:true, crossTable:true) alongside existing
afterId + nextAfterId from ADR-0195. Human-format hint
'Page full — previous page: crossengin retention diff-timeline
--before-id <id> ...' appended when nextBeforeId !== null;
both forward + backward hints can appear in same output when
both cursors are populated mirroring ADR-0196. Use cases
unblocked — page reversal on the timeline surface after
overshooting with --after-id, anchor-based bidirectional audit
on cohort timeline (--after-id <X> + --before-id <X> two
queries one each direction with --add-tenant), cross-table
backward audit walking back through multi-table timeline to
find when policies were originally established, resume from
saved cursor in either direction. Rejected alternatives —
reverse ORDER BY on --before-id (would confuse operators
expecting ASC matches ADR-0189/0195 consistent ordering),
reject both flags at adapter boundary (overzealous substrate-
level range-cursor is sound), single --cursor + --direction
flag (verbose explicit named flags clearer matches ADR-0196/
0195/0175 precedent), use LAST entry id for nextBeforeId
(would point at newest entry useless for backward toward-
older pagination), compute nextBeforeId only when --before-id
explicitly set (would hide bidirectional cursors on first
page; both cursors every full page matches ADR-0196 convention),
auto-paginate via streaming generator (operators want explicit
control), nextBeforeId named prevBeforeId in envelope
(inconsistent with nextAfterId), two separate adapter methods
per direction (3 methods × 2 directions = 6 structural
explosion). Drawbacks — three adapter methods each gain
identical cursor logic for the second time (after ADR-0195's
--after-id) structural symmetry mirrors established pattern
acceptable maintenance cost, adapter accepts both afterId +
beforeId simultaneously but CLI rejects substrate-flexibility/
CLI-restriction split documented, both nextAfterId +
nextBeforeId populated on every full page matches ADR-0196
convention (first-page nextBeforeId returns empty on follow-up
correct end-of-pagination signal in backward direction), no
CLI-side UUID validation matches deferred decision across
the cursor work, cursor operators flipped vs retention history
(`<` here vs `>` on history for --before-id) operators reading
both surfaces may find inversion mildly confusing (mitigated
by consistent semantic labeling regardless of underlying SQL
operator). 9 new adapter tests in trace-retention.test.ts in
a new "PostgresTraceRetention diff-timeline --before-id reverse
cursor" describe block — pair-wise --before-id threads as $N
param into compound cursor with < operator (ASC walk-backward
toward older) verified via SQL substring + params, pair-wise
same $N reused for both subquery lookup and tiebreaker via
regex count appears twice, pair-wise omits when not set, N-way
threads positioned after tenant IN list + table param, cross-
table threads positioned after table IN list + tenant param,
pair-wise composes beforeId with afterId in SAME query both
cursor clauses present (range semantic — substrate-level
flexibility preserved), N-way omits when not set, cross-table
omits when not set, pair-wise ORDER BY remains h.occurred_at
ASC + h.id ASC no direction reversal. 12 new CLI tests in
retention.test.ts in a new "runRetention diff-timeline
--before-id" describe block — pair-wise threads beforeId to
adapter, pair-wise omits when not set backward compat verified,
exit 2 when --after-id + --before-id both set mutually
exclusive with explicit error message, N-way threads alongside
--add-tenant, cross-table threads alongside --cross-table,
JSON envelope echoes beforeId when set (pair-wise), JSON
envelope beforeId=null when not set, JSON nextBeforeId is
FIRST entry id when length === limit verifying ASC ordering
oldest-first (with nextAfterId verifying both populated and
pointing at correct boundaries), JSON nextBeforeId=null when
length < limit, human-format prints previous-page hint with
--before-id <id> when length === limit, JSON envelope echoes
beforeId + nextBeforeId on N-way path, JSON envelope echoes
beforeId + nextBeforeId on cross-table path. cli.ts helpText
extended with [--before-id <uuid>] flag note on both diff-
timeline usage lines + mutual-exclusivity note + nextAfterId
+ nextBeforeId bidirectional navigation. ADR-0197 documents
the design + 8 rejected alternatives + 7 future Qs (--range
range-cursor flag at CLI boundary leveraging adapter's both-
cursors-set support same Q as ADR-0196 Q2, --before-id on
retention diff-history, cross-process cursor stability,
--watch flag for stream-tail mode, --include-cursor-row for
>=/<= semantic, composite indexes for scale, unified
pagination flag set across surfaces noted as no-action-needed
since flag names already unify). The retention CLI now has
18 actions with bidirectional cursor pagination uniform
across BOTH audit-log surfaces — retention history
--after-id + --before-id from ADR-0175 + ADR-0196 (DESC)
and retention diff-timeline --after-id + --before-id from
ADR-0195 + this milestone (ASC across all three dispatch
paths) — operators have stable forward + backward cursor-
paginated workflows on every audit-log query in the retention
substrate without OFFSET drift under concurrent inserts.
M6.7.zz.tenant.opt-out.history.before-id
closes ADR-0175 Q1 by adding `--before-id <uuid>` reverse cursor
pagination to `retention history` — walking BACKWARD through the
DESC-ordered audit log toward newer entries. After ADR-0175
shipped `--after-id` forward pagination (DESC ordering walks
backward in time toward older entries) and ADR-0195 just shipped
`--after-id` across all three diff-timeline paths, operators now
have genuine bidirectional pagination needs — resume from saved
cursor in either direction, page reversal after overshooting,
sliding-window audit anchored on a specific event id, stream-
tail polling for newer events. CLI surface — `--after-id` and
`--before-id` are mutually exclusive at the CLI boundary (both
set → exit 2 with clear error); both compose with every existing
filter (--tenant + --table + --kind + --actor-id +
--with-actor-names + --since + --until + --limit). Adapter
ListOptOutHistoryInput gains optional beforeId?: string field;
adapter accepts BOTH afterId and beforeId simultaneously
(substrate-level range-cursor semantic preserved for
programmatic consumers) while CLI rejects both — documented
flexibility/restriction split. SQL — compound cursor
`(h.occurred_at, h.id) > ((SELECT occurred_at FROM ... WHERE
id = $N), $N)` flips ADR-0175's `<` operator since walking
forward (up the DESC list) means returning NEWER entries; same
inline subquery + tiebreaker pattern. ORDER BY remains
unchanged at `h.occurred_at DESC, h.id DESC` regardless of
cursor direction — operators see latest entries first in every
page. nextBeforeId computed at CLI layer as the FIRST entry's
id when entries.length === limit (boundary for going-newer in
DESC ordering); when entries.length < limit, no more newer
entries exist nextBeforeId is null. JSON envelope gains
beforeId: string | null + nextBeforeId: string | null fields
alongside existing afterId + nextAfterId. Human-format adds
'Page full — previous page: crossengin retention history
--before-id <id> ...' hint when nextBeforeId !== null. Both
cursor fields can be populated when page is full — operators
navigate either direction from any page; on first-page queries
the nextBeforeId points at the absolute newest entry and
follow-up --before-id <that> returns empty (correct end-of-
pagination signal in backward direction); matches GitHub REST
Link-header convention. Use cases unblocked — page reversal
after walking back too far (--before-id <first-id-from-current-
page>), stream-tail mode (polling --before-id <latest-known>
every 30s discovers new events without re-scanning), sliding-
window audit anchored on a specific event id (two queries one
each direction), resume from saved cursor in either direction.
Rejected alternatives — flip ORDER BY direction on --before-id
(would confuse operators expecting DESC; single consistent
ORDER BY matches convention), reject both --after-id and
--before-id at adapter boundary (overzealous substrate-level
range-cursor is sound), single --cursor + --direction flag
(verbose explicit named flags clearer), nextBeforeId named
prevBeforeId (abbreviates inconsistently with nextAfterId), use
LAST entry id for nextBeforeId (would point at oldest entry
useless for backward pagination), compute nextBeforeId only
when --before-id explicitly set (would require operators know
direction availability before trying), reverse-direction
reverse-mode with --before-id + ASC ordering (semantic stretch
DESC is expected), server-side opaque cursor encoding (UUID is
simple). Drawbacks — adapter accepts both flags simultaneously
but CLI rejects (substrate-flexibility/CLI-restriction split
documented), both nextAfterId + nextBeforeId populated on every
full page (first-page nextBeforeId points at absolute newest
entry follow-up returns empty as end-of-pagination signal), no
--before-id on retention diff-timeline this milestone (defer
to future session), no CLI-side UUID validation (matches
ADR-0175/0186/0193/0195 deferred decision), nextBeforeId best-
effort under concurrent inserts (walked correctly on next
backward page via compound cursor), DESC ordering constant
regardless of cursor direction (matches GitHub REST and most
paginated APIs convention). 6 new adapter tests in trace-
retention.test.ts in a new "PostgresTraceRetention.listOptOutHistory
--before-id reverse cursor" describe block — threads as $N
param into compound cursor with > operator verified, same $N
reused for both subquery lookup and tiebreaker verified via
regex count appears twice, omits when not set, composes with
afterId in SAME query both cursor clauses present + both
params threaded (range semantic), composes with all 7 other
filters with afterId positioned before beforeId in WHERE
clause order, ORDER BY remains h.occurred_at DESC + h.id DESC
no direction reversal. 10 new CLI tests in retention.test.ts
in a new "runRetention history --before-id" describe block —
threads beforeId to adapter, omits when not set backward
compat, exit 2 when --after-id + --before-id both set mutually
exclusive with explicit error, JSON envelope echoes beforeId
when set, JSON envelope beforeId=null when not set, JSON
nextBeforeId is FIRST entry id when length === limit (with
nextAfterId verifying both populated), JSON nextBeforeId=null
when length < limit, human-format prints previous-page hint
when length === limit, human-format omits hint when length <
limit, composes with all 7 other filters (--tenant + --table
+ --kind + --actor-id + --since + --until + --before-id +
--limit). cli.ts helpText extended with [--before-id <uuid>]
flag note on retention history usage line + 5-line description
explaining bidirectional pagination + mutual exclusivity with
--after-id + nextAfterId + nextBeforeId bidirectional
navigation when page is full. ADR-0196 documents the design +
8 rejected alternatives + 7 future Qs (--before-id on retention
diff-timeline all three paths, --range <after-id>..<before-id>
range-cursor flag at CLI boundary leveraging adapter's both-
cursors-set support, --before-id on retention diff-history,
cross-process cursor stability via point-in-time snapshot,
--watch flag auto-polling for stream-tail mode, --include-
cursor-row for >=/<= semantic, composite indexes). The
retention CLI now has 18 actions with bidirectional cursor
pagination on the canonical audit-log surface — operators
walk forward (--after-id, older) and backward (--before-id,
newer) on retention history symmetrically without OFFSET
drift under concurrent inserts; diff-timeline still has
forward-only --after-id from ADR-0195 with --before-id
deferred to future session.
M6.7.zz.tenant.opt-out.cli.diff-timeline.cursor
...
`--add-tenant` on the timeline axis; matches the cohort-
investigation pattern operators already learned. New
diffHistoryTimelineNway adapter separate from
diffHistoryTimeline mirroring ADR-0183's separate-method
pattern — pair-wise narrow TypeScript types (tenantSide:
"A" | "B") preserved for the common case; N-way gets its own
NwayTimelineEntry with tenantLabel: string field. SQL uses
h.tenant_id IN ($1, $2, ...) instead of OR; PG handles the
IN-list efficiently with existing (table_name, occurred_at)
index. Per-row tenantLabel tagged at adapter via Map<tenantId,
label> built from input order; duplicate tenantIds — first
occurrence wins (matches ADR-0183 dedup). New labelForIndex
exported helper assigns 'A'..'Z' then 'T27'/'T28'/... matching
ADR-0183/0188 cohort label convention. Dispatcher in
runRetentionDiffTimeline reads --add-tenant via getMultiFlag,
branches to N-way path when set, falls through to existing
pair-wise path otherwise; all other flags (--since, --until,
--limit, --with-actor-names) compose uniformly across both
paths. JSON envelope gains nway:true discriminator
distinguishing N-way from pair-wise (which has no
discriminator); operators jq-filter on .result.entries[] |
select(.tenantLabel == "C") to extract one tenant's events
from the merged cohort timeline. Human output renders 'N-way
timeline for N tenants on <table>:' header + per-tenant
'Tenant A: <uuid>' rows + 'Events (N):' header + per-event
'<occurred_at> [<label>] <event-kind-padded> <state-summary>
[by <actor>]' lines. New formatTimelineNwayDiff exported
helper reuses summarizeTimelineEntry + formatActor — no
duplication. Use cases unblocked — cohort-wide divergence
audit (5-tenant cohort in one command + jq group_by per
tenant), legal-hold cohort verification across N hold-
tenants in single command, tier migration drift detection
filtering on tenantLabel via jq, compliance attestation
across regulated cohort × prunable tables via shell loop.
Rejected alternatives — widen diffHistoryTimeline to
ReadonlyArray<string> of length 2..N (loses pair-wise
TypeScript narrowing for tenantSide literal), N-way via
--tenants <a,b,c> comma-separated single flag (fragile),
auto-promote to N-way when >3 positional args (magical),
new action retention diff-timeline-nway (adds CLI surface),
use tenantSide on N-way entries widened to string (less
honest about cross-axis semantic — tenantLabel matches
ADR-0188 labels rename), JSON envelope without nway
discriminator (operators detect by tenantIds[] vs
tenantIdA/tenantIdB shape — explicit discriminator clearer),
N-way + --vs-platform synthetic comparison (different
semantic defer), state-replay variant emitting per-event
divergence delta (significant query cost defer),
--exclude-tenant set-subtraction (defer). Drawbacks — two
adapter methods to maintain (mirrors ADR-0183 pattern;
pair-wise narrow TypeScript types preserved), tenantSide
literal union vs tenantLabel widened string field name
difference (callers branch on nway discriminator first
acceptable), label collision at >26 tenants (T27 less
readable but rare), no input tenantIds deduplication
(matches ADR-0183; first-occurrence-wins is documented
behavior), no N-way × --cross-table matrix (defer different
semantic axis), PG IN-list parser limit at thousands of
tenants (defer chunking until measured slow), no state-
replay variant (operators wrap with `retention diff
--add-tenant` for current state). 11 new adapter tests in
trace-retention.test.ts (rejects fewer than 2 tenantIds
with 'at least 2 tenantIds required' error, issues single
query with h.tenant_id IN ($1, $2, $3) for 3 tenants
verified via SQL substring + params, tags each entry with
tenantLabel A/B/C from input order, assigns T27/T28 for
indices beyond 26, ORDER BY h.occurred_at ASC + h.id ASC
chronological, returns empty entries when no rows, threads
--since/--until/--limit through with param positions
after tenant IN list, composes with joinActor=true emitting
LEFT JOIN + actor cols, rejects limit < 1, throws on
unknown event_kind defensive schema-drift guard, preserves
duplicate tenantIds first-occurrence wins label). 8 new
CLI tests in retention.test.ts — dispatches to N-way path
when --add-tenant present, collects multiple --add-tenant
flags in argv order, does NOT call N-way method when flag
absent backward compat, threads --with-actor-names + --since
+ --limit through, human-format 'N-way timeline for 3
tenants on <table>:' header, human-format renders [A]/[B]/[C]
tagged events with '(policy deleted)' for null nextState,
JSON envelope includes nway:true + result.tenantIds +
entries with tenantLabel, adapter errors propagate exit 1.
3 new formatTimelineNwayDiff helper unit tests — 'No
history events for any of these tenants' empty rendering,
[A]/[B]/[C] tagged event lines with state summary, 'by
Alice Smith (uuid)' suffix with withActorNames=true opt.
cli.ts helpText extended with --add-tenant flag note + N-way
semantic description. ADR-0191 documents the design + 9
rejected alternatives + 8 future Qs (N-way + --cross-table
matrix, N-way + --vs-platform, --exclude-tenant,
--add-tenant <slug> via meta.tenants, cursor pagination
via --after-id matching ADR-0175, state-replay per-event
divergence delta, tagged-union JSON envelope across all
retention diff-* actions, auto-deduplicate tenantIds at
CLI boundary). The retention CLI now has 18 actions with
N-way cohort comparison spanning both axes — point-in-time
diff via retention diff --add-tenant from ADR-0183 +
chronological diff via retention diff-timeline --add-tenant
from this milestone — operators have first-class cohort-
investigation ergonomics across both temporal axes.
M6.7.zz.tenant.opt-out.cli.diff-timeline.with-actor-names
...
history then jq-filtered — current workaround
`--format json | jq '.entries[] | select(.actorId == "...")'`
broke LIMIT correctness (asking for --limit 100 returned
<100 entries after jq filtered) and pagination correctness
(--after-id semantics with jq-side filter are wrong);
substrate-side WHERE filter fixes both. Adapter
ListOptOutHistoryInput gains optional actorId?: string
field positioned between eventKind and since in WHERE
clause assembly (param sequence $1:tenantId, $2:tableName,
$3:eventKind, $4:actorId, $5:since, $6:until, $7:afterId,
$8:limit when all set); when set SQL adds h.actor_id = $N
condition + index-scan on existing actor_id column; when
omitted no actor filter applied existing behavior
preserved. Composes uniformly with every existing filter
dimension — --tenant + --actor-id (actor's mutations on
one tenant), --table + --actor-id (one table), --kind +
--actor-id (specific event kind), --since/--until +
--actor-id (time range), --after-id + --actor-id
(paginated actor-scoped history), --with-actor-names +
--actor-id (actor-scoped + display-name surfacing
canonical "Alice's audit log" pattern). JSON envelope
gains actorId field (string when set, null when not)
position between eventKind and since matches WHERE-clause
order. No null-actor filtering — operators wanting
actor_id IS NULL events jq-filter on .actorId === null
(substrate stays minimal one filter per dimension no
sentinel values null is logically absence of actor not
specific actor ID jq idiom is one-liner). Pure CLI
delivery modulo one adapter input field — no new types
no new helpers no result-shape changes mechanical
addition. Use cases unblocked — per-actor incident
response (--actor-id <suspect> --with-actor-names --since
2026-05-01), compliance attestation per officer (for loop
through compliance-officers.txt jq aggregating), "what
did Alice do in the last 7 days?" (--actor-id <uuid>
--with-actor-names --since 7d-ago reads like English),
CI gate on unauthorized-actor mutations (--actor-id
service-account --since build-start jq length > 0 fails
build). Rejected alternatives — CLI-side jq filtering as
documented workflow (rejected — substrate-side ensures
LIMIT + pagination correctness; jq-side returns < N
entries when filtering bites), --actor-id-equals /
--actor-id-not pair (overkill for v1 positive-only is
common case; defer not-equals), multi-actor --actor-id
repeated via multiFlags (operators wanting OR across N
actors; pair-wise sufficient for now run twice + concat),
--actor system sentinel for null filtering (overloads
string semantic; operators jq-filter), add actor_id
composite index (schema migration concern defer until
measured slow), --actor-id-pattern regex (operators with
structured ID schemes uncommon defer), implicit actor
scoping via --my or $CROSSENGIN_ACTOR_ID env var
(operator-state-dependent magic; explicit --actor-id
clearer), surface in JSON as actorFilter for naming
consistency with tenantFilter/tableFilter (chose actorId
because per-entry actorId field already uses that name;
envelope-level matches entry-level). Drawbacks — no
null-actor sentinel operators wanting actor_id IS NULL
filtering jq-filter (documented), no --actor-id-not
exclusion operators excluding one actor jq-filter
(defer), no multi-actor filter (--actor-id alice
--actor-id bob with OR semantics; operators wanting that
run two commands concatenate defer), no --actor-name-
equals for human-readable input (requires actor → UUID
resolution operators look up UUIDs first; future Q
paired with ADR-0185 Q2), substrate-side index
requirement actor_id filtering without composite index
(actor_id, occurred_at) does sequence scan + sort defer
until measured slow meta-schema currently has no
actor_id index, no validation of UUID shape at CLI
boundary operators passing invalid UUIDs hit PG error
message rather than CLI exit-2 (matches existing
pattern from --after-id ADR-0175 deferred validation
to PG). 7 new adapter tests in trace-retention.test.ts
(adds h.actor_id = $N WHERE clause when actorId set
verified via SQL substring + params, omits actor_id
WHERE when actorId not set, composes with tenantId
(both h.tenant_id = $1 + h.actor_id = $2 verified),
composes with joinActor (LEFT JOIN + actor-id WHERE
both present), composes with ALL filter dimensions
(tenant + table + kind + actor + since + until +
afterId + limit — full param array verified), returns
only rows matching actor verified via mock row +
adapter result actorId check, returns empty when
filtered actor has no rows); 7 new CLI tests in
retention.test.ts (threads actor-id to adapter when
--actor-id set, omits actorId from adapter input when
not set backward compat, composes with --tenant +
--actor-id + --kind, composes with --with-actor-names
verifying both actorId + joinActor:true threaded, JSON
envelope includes actorId field with value when set,
JSON envelope actorId null when not set, human-format
empty-result message preserved when --actor-id has no
matches). cli.ts helpText extended with --actor-id
<uuid> flag note added to retention history usage line
+ description mentioning actor filtering for one
actor's mutations. ADR-0186 documents the design + 8
rejected alternatives + 7 future Qs (--actor-name-
equals via meta.users.display_name JOIN pairing with
ADR-0185, multi-actor via repeated --actor-id using
multiFlags from ADR-0183, --actor-id-not exclusion,
composite index on (actor_id, occurred_at) for large-
scale pagination, CLI-side UUID validation matching
ADR-0175 deferred decision, --system-only flag for
actor_id IS NULL, surface --actor-id in audit-log diff
actions like retention diff-history --actor-id).
The retention CLI now has 17 actions with 6 filter
dimensions on retention history (tenant + table +
kind + actor + since + until) plus cursor pagination
plus --with-actor-names display surfacing —
comprehensive per-actor forensic + compliance
workflows now CLI-native.
M6.7.zz.tenant.opt-out.history.actor-join
...
review),
ADR-0186 covers M6.7.zz.tenant.opt-out.cli.history.actor-filter
(`retention history --actor-id <uuid>` filter — closes
ADR-0185 Q1 — ListOptOutHistoryInput gains actorId?:
string field positioned between eventKind and since;
substrate-side WHERE filter h.actor_id = $N ensures
LIMIT + pagination correctness vs jq-side post-fetch
filtering; composes with all 5 existing filter
dimensions + cursor pagination + --with-actor-names
...
timeline surfaces),
ADR-0191 covers M6.7.zz.tenant.opt-out.cli.diff-timeline.add-tenant
(`retention diff-timeline --add-tenant <c>
[--add-tenant <d> ...]` N-way cross-tenant chronological
merge — closes ADR-0189 Q1 — mirrors ADR-0183's
--add-tenant on the timeline axis; new
diffHistoryTimelineNway adapter separate from
diffHistoryTimeline (mirrors ADR-0183 separate-method
pattern); SQL uses h.tenant_id IN ($1, $2, ...) PG
handles efficiently with existing (table_name,
occurred_at) index; per-row tenantLabel tagged at
adapter via Map<tenantId, label> built from input
order; new labelForIndex exported helper assigns A..Z
then T27/T28/... matching ADR-0183/0188 cohort labels;
dispatcher in runRetentionDiffTimeline branches on
--add-tenant presence backward compat for pair-wise
path preserved; JSON envelope gains nway:true
discriminator; operators jq-filter on entries[] |
select(.tenantLabel == "C") to extract one tenant's
events; new formatTimelineNwayDiff exported helper
reuses summarizeTimelineEntry + formatActor no
duplication; all flags (--since, --until, --limit,
--with-actor-names) compose uniformly across both
...
ADR-0196 covers M6.7.zz.tenant.opt-out.history.before-id
(`retention history --before-id <uuid>` reverse
cursor pagination — closes ADR-0175 Q1 — walks
BACKWARD through DESC-ordered audit log toward
NEWER entries (vs --after-id forward toward older);
compound cursor `(h.occurred_at, h.id) > ((SELECT
occurred_at FROM ... WHERE id = $N), $N)` flips
ADR-0175's `<` operator since walking up the DESC
list returns newer entries; ListOptOutHistoryInput
gains optional beforeId?: string field; adapter
accepts both afterId + beforeId simultaneously
substrate-level range-cursor semantic preserved
while CLI enforces mutual exclusivity (--after-id
+ --before-id both set → exit 2); ORDER BY remains
DESC unchanged regardless of cursor direction
matches GitHub REST convention; nextBeforeId
computed at CLI layer as FIRST entry id when
length === limit (boundary for going-newer);
JSON envelope echoes beforeId + nextBeforeId;
human-format adds 'Page full — previous page:
--before-id <id>' hint; page reversal + stream-
tail polling + sliding-window audit + resume from
saved cursor either direction workflows now CLI-
native),
ADR-0197 covers M6.7.zz.tenant.opt-out.cli.diff-timeline.before-id
(`retention diff-timeline --before-id <uuid>`
reverse cursor pagination across all three diff-
timeline paths — closes ADR-0195 Q1 + ADR-0196
Q1 — walks BACKWARD through ASC-ordered timeline
toward OLDER entries (vs --after-id forward toward
newer); cursor SQL flips operator from ADR-0195's
`>` to `<` since diff-timeline orders ASC and
walking up the list returns older entries — mirror
of ADR-0196's `<`/`>` flip on the DESC retention
history surface; operator semantics consistent
across both surfaces — --after-id always means
"next chronologically-forward page" and
--before-id always "previous chronologically-
backward page" regardless of underlying SQL
operator; all three diff-timeline input types gain
optional beforeId?: string field; adapter accepts
both afterId + beforeId simultaneously substrate-
level range-cursor semantic preserved across all
three paths while CLI enforces mutual exclusivity;
nextBeforeId computed at CLI layer for all three
paths as FIRST entry id when length === limit
(boundary for going-older in ASC ordering);
JSON envelope echoes beforeId + nextBeforeId
across all three envelope shapes; human-format
hint appended when nextBeforeId non-null; page
reversal + anchor-based bidirectional audit on
...
now CLI-native),
ADR-0199 covers M6.7.zz.tenant.opt-out.cli.diff-timeline.multi-actor
(`retention diff-timeline --actor-id <uuid>`
repeatable for OR-semantic multi-actor filter —
closes ADR-0193 Q1 — multiFlags infrastructure
from ADR-0183/0188 reused across all three diff-
timeline dispatch paths; breaking adapter type
rename actorId?: string → actorIds?:
ReadonlyArray<string> on all three input types
(contained scope no external consumers; one-shot
clean break beats permanent two-field surface
matches ADR-0183's tenantIds + ADR-0188's
tableNames N-way collection pattern); SQL builds
positional placeholders for IN clause (single-
element IN($N) equivalent to old = $N PG handles
both identically; multi-element IN($N1, $N2,
...) yields OR semantic); CLI getMultiFlag
("actor-id") returns string[]; JSON envelope
renames actorId field to actorIds on all three
envelope shapes; retention history's actorId
stays SINGLE value intentionally with divergence
documented (separate milestone could widen if
demand emerges); two-actor cohort forensics +
service-account allowlist audit + per-team
mutation audit + compliance reviewer audit set
workflows now CLI-native; existing actor-filter
test block rewritten + expanded from 7→11
adapter tests and 8→11 CLI tests to cover both
single and multi-actor cases across all three
dispatch paths),
ADR-0200 covers M6.7.zz.tenant.opt-out.cli.diff-timeline.multi-kind
(`retention diff-timeline --kind <event-kind>`
repeatable for OR-semantic multi-kind filter —
closes ADR-0194 Q1 — mirror of ADR-0199's multi-
actor shape applied to the kind filter dimension
across all three diff-timeline dispatch paths;
breaking adapter type rename eventKind?:
OptOutHistoryEventKind → eventKinds?:
ReadonlyArray<OptOutHistoryEventKind> matching
ADR-0199's one-shot break pattern; CLI uses
getMultiFlag("kind") + per-value validation
against 4-value OPT_OUT_HISTORY_EVENT_KINDS tuple
(exit 2 with offending value named on any
invalid); single → IN ($N), multi → IN ($N1,
$N2, ...); JSON envelope renames kind to kinds
on all three diff-timeline envelope shapes;
retention history + retention diff-history keep
eventKind: single intentionally (history is
list-style with single-kind filter common;
diff-history is per-event-pair expectation check
ADR-0198 Q2 covers multi-kind there); opt-out
lifecycle audit + per-actor per-kinds forensics
+ positive exclusion of noise kinds + compliance
attestation across cohort × kinds workflows now
CLI-native; existing kind-filter test block
...
ADR-0202 covers M6.7.zz.tenant.opt-out.cli.diff-timeline.range
(`retention diff-timeline --range <after-id>..<before-id>`
window-cursor flag across all three diff-timeline
dispatch paths — closes ADR-0197 Q1 + ADR-0201
Q1 — mirror of ADR-0201's pattern on the
diff-timeline surface; pure CLI delivery (no
adapter changes since all three diff-timeline
adapter inputs already accept both afterId +
beforeId simultaneously from ADR-0195/0197 with
CLI gating now lifted via --range gesture); same
`..` separator + parse validation + mutual
exclusivity rules + bare-flag-error-points-at-
--range pattern; JSON envelope gains range:
string | null field on all three envelope shapes
(pair-wise no-discriminator, nway:true,
crossTable:true); parallel implementation across
three paths mirrors how every other diff-timeline
filter already exists three times; window
semantics depend on ASC ordering (diff-timeline
returns events newer than a AND older than b —
INVERSE direction from retention history's DESC
range semantic documented in helpText); anchor-
based bidirectional cohort audit + cross-table
window scoped to migration period + per-actor
per-window forensic narrowing + CI gate "no
mutations in protected window" workflows now
CLI-native across all three diff-timeline paths).
ADR-0203 covers M6.7.zz.tenant.opt-out.cli.diff-history.actor-filter
(`retention diff-history --actor-id <uuid>` actor
expectation check — closes ADR-0198 Q3; mirror of
ADR-0198's `--kind` pattern on the actor dimension;
diff-history takes exactly two IDs not a list query
so --actor-id semantic shifts from filter to
expectation check matching ADR-0198 design — operator
declares "both events were authored by actor X" and
adapter throws when reality differs; adapter SELECT
widens to include actor_id column + result row type
gains actor_id: string | null + new actorId
expectation check after the existing eventKind check
throws 'expected both events to have actor_id <X> but
A is <Y> and B is <Z>' on mismatch naming offending
side(s); null actor_id renders as <system> matching
ADR-0185 retention history convention; CLI threads
--actor-id <uuid> via getStringFlag (no UUID
validation matching ADR-0175/0186/0193 deferred
decision), passes actorId to adapter call, echoes
actorId: string | null in JSON envelope matching
ADR-0193 convention; composes with --kind from
ADR-0198 — both expectations applied independently
either mismatch throws; mismatch exits 1 (runtime
path matching cross-tenant + cross-table + eventKind
mismatch error propagation), invalid input exits 2
(CLI misuse); use cases unblocked — compliance script
asserting actor match, forensic analysis gated on
actor (compose with --kind for fine-grained gates),
CI gate verifying service-account migration, detect
inadvertent cross-actor comparison; 7 new adapter
tests in a new "PostgresTraceRetention.diffHistoryEntries
--actor-id expectation check" describe block (accepts
both-match, throws on A-mismatch, throws on B-mismatch,
throws naming-both-sides including <system> rendering
for null actor_id, renders <system> for null actor_id
in mismatch message, omits check when actorId not set
backward compat, composes with eventKind check both
pass); 6 new CLI tests in a new "runRetention diff-
history --actor-id" describe block (threads actorId
when --actor-id set, omits actorId when --actor-id
NOT set backward compat, composes with --kind
threading both eventKind and actorId, adapter mismatch
error propagates exit 1, JSON envelope echoes actorId
when set, JSON envelope actorId=null when NOT set);
cli.ts helpText extended for retention diff-history
usage line with [--actor-id <uuid>] flag + 2-line
description; future Qs cover per-side --actor-id-a +
--actor-id-b for asymmetric expectations, --actor-id
<a>|<b>|<c> multi-value expectation different shape
from multi-actor OR-filter, --with-actor-names on
diff-history LEFT JOIN meta.users for both events'
actors, --actor-name-equals filter via meta.users
lookup pairing with ADR-0186 Q3, CLI-side UUID
validation matching ADR-0175 deferred decision,
--system-only flag for both events being system-
authored. The retention CLI now has 18 actions with
--actor-id support uniform across THREE retention
diff family members — retention history (filter from
ADR-0186), retention diff-timeline (filter from
ADR-0193/0199 across all three paths), retention
diff-history (expectation check from this milestone)
— semantics adapt to surface but flag name + UUID
input + helpText behavioral note are consistent
across the family).
ADR-0204 covers M6.7.zz.tenant.opt-out.cli.diff-history.with-actor-names
(`retention diff-history --with-actor-names` actor
display name surfacing — closes ADR-0203 Q3 +
ADR-0198 Q5; composes ADR-0185's LEFT JOIN
meta.users infrastructure with diff-history surface;
DiffHistoryEntriesInput gains joinActor?: boolean;
when set adapter widens SELECT to include
u.display_name + u.email via LEFT JOIN on actor_id;
result type gains required actorIdA + actorIdB
fields (always populated from existing actor_id
SELECT column since ADR-0203) and optional
actorDisplayNameA/B + actorEmailA/B fields populated
only when joinActor=true; SQL aliased h. consistently
matching ADR-0185/0190 pattern; CLI threads
--with-actor-names via getBooleanFlag passes
joinActor to adapter passes opts.withActorNames to
formatHistoryDiff which appends 'by <actor>' suffix
on each event line using generalized formatActor
helper from ADR-0190 (display_name (uuid) → email
(uuid) → raw uuid → <system> for null actor_id);
composes with --kind (ADR-0198) + --actor-id
(ADR-0203) without restriction; JSON envelope gains
withActorNames discriminator + result conditionally
carries display fields when set; minor breaking
change in DiffHistoryEntriesResult — actorIdA +
actorIdB now required where they were absent in
ADR-0173 (contained scope reflects always-available
data); use cases unblocked — one-command "who made
each change" diff audit, compliance review with
named actor attribution, compose with --actor-id +
--with-actor-names for forensic discipline, incident
post-mortems with named actor timelines; 8 new
adapter tests in a new "PostgresTraceRetention.diffHistoryEntries
--with-actor-names" describe block (omits JOIN
backward compat, emits LEFT JOIN meta.users with
display_name + email cols, returns actor info both
events, returns null for orphan FK, null for system
actor, omits display fields when joinActor false,
always populates actorIdA + actorIdB even without
join, composes with --actor-id expectation check);
9 new CLI tests in a new "runRetention diff-history
--with-actor-names" describe block (threads
joinActor when --with-actor-names set, omits
joinActor backward compat, human-format renders 'by
Name (uuid)' for both events, <system> for null
actor_id, email fallback when display_name null,
omits 'by ...' suffix when flag NOT set, JSON
envelope withActorNames + actor fields when set,
JSON withActorNames=false when NOT set, composes
with --actor-id + --kind); cli.ts helpText extended
with [--with-actor-names] flag note + 3-line
description; future Qs cover --actor-name-equals
filter, show user status, display tenant_membership
role, per-field actor attribution, --actor-id-not
exclusion to compose with display, apply pattern
uniformly to other diff-* surfaces. The retention
CLI now has 18 actions with --with-actor-names
support uniform across THREE retention audit-log
surfaces — retention history (ADR-0185), retention
diff-timeline (ADR-0190 across all three paths),
retention diff-history (this milestone) — operators
...
surface).
ADR-0206 covers M6.7.zz.tenant.opt-out.cli.history.actor-not
(`retention history --actor-id-not <uuid>` substrate-
side WHERE NOT filter — closes ADR-0186 Q1 + ADR-0205
Q6; inverse of ADR-0186's positive --actor-id filter
on list-style audit-log surface; different semantic
from ADR-0205's exclusion expectation check on diff-
history (which throws on match in 2-fixed-event
compare) — this is a list-style query filter
excluding rows where actor_id matches; ListOptOutHistoryInput
gains optional actorIdNot?: string field; SQL adds
`(h.actor_id IS NULL OR h.actor_id != $N)` WHERE
clause explicitly OR'd IS NULL so system events
INCLUDED (PG actor_id != $N returns NULL for null
values silently filtering systems out which is wrong
intent); substrate-side filter for LIMIT correctness
+ cursor-pagination correctness + index usage parity;
CLI threads via getStringFlag, echoes actorIdNot:
string | null in JSON envelope; NOT mutually exclusive
with --actor-id at CLI boundary; contradictory
--actor-id X --actor-id-not X returns empty silently
(natural outcome); no CLI-side UUID validation; use
cases — service-account-free human audit, suspended-
user activity audit, self-exclude review, compliance
non-automation filter; 8 new adapter tests (adds
WHERE clause with IS NULL OR != verified, omits when
not set, includes system events (null actor_id),
composes with tenantId + actorId both threaded though
SQL-contradictory + joinActor + all-filter-dimensions,
returns rows excluding the actor); 6 new CLI tests
(threads actorIdNot, omits backward compat, composes
with --actor-id independently, composes with --tenant
+ --with-actor-names + --actor-id-not 3-flag, JSON
envelope echoes when set, JSON null when NOT set);
cli.ts helpText extended with [--actor-id-not <uuid>]
flag note + 2-line description; future Qs cover
multi-value via repeated flag using multiFlags from
ADR-0183, --system-only + --no-system explicit null
matching, --actor-id-not on diff-timeline, --kind-not
companion, composite index for scale, --actor-name-
not via meta.users JOIN. The retention CLI now has
18 actions with --actor-id-not support spanning TWO
retention audit-log surfaces — retention diff-history
(expectation check from ADR-0205) and retention
history (substrate-side filter from this milestone)
— operators get full positive + negative actor
expectations across both cross-event and list-style
query surfaces with surface-appropriate semantics
under one consistent flag name).
ADR-0207 covers M6.7.zz.tenant.opt-out.cli.diff-timeline.actor-not
(`retention diff-timeline --actor-id-not <uuid>`
repeatable multi-value exclusion filter across all
three diff-timeline dispatch paths — closes ADR-0193
+ ADR-0206 Q3; mirror of ADR-0206's retention-history
pattern on diff-timeline surface but multi-value to
match ADR-0199's positive multi-actor filter shape on
diff-timeline (within-surface consistency wins over
cross-surface uniformity); all three input types gain
actorIdsNot?: ReadonlyArray<string>; SQL adds
(h.actor_id IS NULL OR h.actor_id NOT IN ($N1, $N2,
...)) when set non-empty explicitly OR'd IS NULL so
system events INCLUDED; empty array filter-not-set
matching ADR-0199 convention; CLI getMultiFlag
("actor-id-not") repeated flag occurrences threaded
to whichever of three adapters dispatched; JSON
envelope echoes actorIdsNot: string[] | null on all
three envelope shapes positioned after actorIds field;
NOT mutually exclusive with --actor-id both clauses
fire independently at SQL layer contradictory returns
empty silently natural outcome; parallel implementation
across three paths matches established pattern; use
cases unblocked — cohort drift detection excluding
noise actors, cross-table audit excluding automation,
self-exclude reviewer audit, compose positive +
negative for overlapping cohorts; 9 new adapter tests
(pair-wise single + multi NOT IN, omits clause when
not set, empty array filter-not-set, N-way + cross-
table positioned correctly, composes with actorIds +
joinActor + eventKinds); 10 new CLI tests (pair-wise
single + multi threading, omits backward compat, N-way
+ cross-table threading, composes with --actor-id all
three JSON envelopes echo actorIdsNot pair-wise null
when NOT set); cli.ts helpText extended on both diff-
timeline usage lines; future Qs cover --actor-id-not
on diff-history already covered as expectation check
from ADR-0205, widen retention history to multi-value
pairs with ADR-0206 Q1, --system-only + --no-system
explicit null actor matching, --kind-not exclusion
companion, composite index for scale, --actor-name-
not filter via meta.users JOIN. The retention CLI now
has 18 actions with --actor-id-not support spanning
THREE retention audit-log surfaces — retention history
(substrate-side filter from ADR-0206), retention diff-
history (expectation check from ADR-0205), retention
diff-timeline (substrate-side multi-value filter
across all 3 paths from this milestone) — operators
...
ADR-0207).
ADR-0209 covers M6.7.zz.tenant.opt-out.cli.history.system-only
(`crossengin retention history --system-only` /
`--no-system` boolean flag pair — closes ADR-0186 +
ADR-0206 + ADR-0205/0207 future Qs that listed this
across 4 retention ADRs; the retention history audit
log distinguishes two actor kinds — UUID-bearing rows
(human or service-account authored) and actor_id IS
NULL rows (system-initiated substrate background jobs
— prune sweeps, scheduled retention runs, automated
expiry lifts); operators have legitimate "show only X"
and "exclude all X" workflows for both kinds (system-
only audit for substrate-job health monitoring, no-
system audit for compliance attestation excluding
automation, forensic discipline during incidents,
human-only attestation for HIPAA/SOC 2/21 CFR 11);
until this milestone operators jq-filtered post-fetch
breaking LIMIT correctness + cursor-pagination
correctness + index-usage parity — substrate-side
WHERE the only way to get all three right (same
reasoning as ADR-0186 actor filter + ADR-0206 actor-
not filter); CLI surface `--system-only` boolean flag
filters to actor_id IS NULL events; `--no-system`
boolean flag filters to actor_id IS NOT NULL events;
mutually exclusive at CLI boundary both set returns
exit 2 with `--system-only and --no-system are
mutually exclusive` error before any PG query (CI logs
recognize as misuse); composes with every existing
filter without restriction; new exported
ActorPresenceFilter discriminated string union
`"system_only" | "no_system"` over two-boolean
adapter shape because (1) mutual exclusivity enforced
at type level — impossible to express "both at once"
in adapter input, (2) adapter consumers see clean
3-state choice, (3) CLI translates two boolean flags
into single union value after mutual-exclusivity
check; ListOptOutHistoryInput gains optional
actorPresence?: ActorPresenceFilter field; SQL adds
one of two conditional WHERE clauses NO PARAMS needed
(IS NULL / IS NOT NULL are SQL constructs not value
comparisons): system_only adds h.actor_id IS NULL;
no_system adds h.actor_id IS NOT NULL; position
immediately after actorIdNot filter clause before
--since/--until time-range clauses matching existing
actor-dimension grouping in WHERE-clause assembly;
JSON envelope gains two boolean fields systemOnly +
noSystem matching existing withActorNames boolean-
echo convention from ADR-0185 (every CLI flag has
corresponding envelope field with same shape;
operators read literal flag state; mutual exclusivity
at CLI guarantees both never true simultaneously);
composition with existing actor filters — --system-
only + --actor-id is contradictory at data layer
(system events have actor_id IS NULL; --actor-id
requires UUID; substrate returns empty silently
natural outcome), --system-only + --actor-id-not is
redundant but valid (system events are NEITHER any
specific UUID), --no-system + --actor-id is canonical
"human X's mutations" idiom (--no-system redundant
since actor_id = $N implies IS NOT NULL but
harmless), --no-system + --actor-id-not is canonical
"human mutations excluding X" idiom (distinguished
from --actor-id-not alone which includes system
events); use cases unblocked — system-authored
maintenance audit (count system events by kind for
dashboard), human-only compliance attestation
(quarterly audit excluding automation), forensic
incident scoping (was this state change human-
driven?), substrate-job health monitoring (count by
event_kind grouped), compose with actor exclusion
(human mutations excluding migration SA); rejected
alternatives — two separate boolean fields on
adapter (allows invalid both-true state), --actor-
presence single flag with enum value (operators
remember enum vocab vs natural boolean flags),
--system / --human flag names ("human" misleading
since SAs also non-system), --actor-id null sentinel
(overloads --actor-id UUID semantic), CLI-side
--actor-id X + --system-only contradiction error
(would block valid edge cases), substrate-side both-
flag mutual-exclusivity error in adapter (type union
makes invalid unrepresentable), two-value enum
--actor-presence required|excluded (operator vocab
unusual), PG partial index on actor_id IS NULL in
this milestone (premature optimization defer),
--system-only without --no-system companion
(operators wanting exclude system jq-filter post-
fetch with same LIMIT/pagination correctness problems;
ship both for symmetry), default --no-system=true
silent behavior change for existing callers backward-
incompatible; 8 new adapter tests in a new
"PostgresTraceRetention.listOptOutHistory
actorPresence filter" describe block — adds
h.actor_id IS NULL WHERE clause when system_only,
adds h.actor_id IS NOT NULL WHERE clause when
no_system, omits clause when not set, adds no params
for IS NULL/IS NOT NULL clauses no placeholder
pollution LIMIT param position correctly assigned,
composes with tenantId + tableName (system_only),
composes with actorIdNot + no_system (redundant but
valid both clauses present), composes with joinActor
+ actorPresence (LEFT JOIN + IS NULL both present),
returns rows with null actor_id when system_only
matches; 9 new CLI tests in a new "runRetention
history --system-only / --no-system" describe block
— exit 2 when both flags set with explicit error
message, threads actorPresence='system_only' to
adapter when --system-only set, threads
actorPresence='no_system' when --no-system set,
omits actorPresence when neither flag set backward
compat, composes with --tenant + --system-only,
composes with --actor-id-not + --no-system both
threaded independently, JSON envelope echoes
systemOnly=true + noSystem=false when --system-only
set, JSON envelope echoes noSystem=true +
systemOnly=false when --no-system set, JSON envelope
echoes both false when neither set; cli.ts helpText
extended for retention history usage line with
[--system-only | --no-system] notation indicating
mutual exclusivity + 3-line description added
explaining IS NULL / IS NOT NULL semantic + mutual
exclusivity. Future Qs cover --system-only/--no-
system on diff-history as expectation check ("both
events must be system" / "neither is system") pairing
with ADR-0205 Q3 deferred, --system-only/--no-system
on diff-timeline across all 3 dispatch paths pairing
with ADR-0193 + ADR-0207 future Qs deferred, partial
index on actor_id IS NULL for scale defer until
measured, short-circuit LEFT JOIN when
actorPresence='system_only' since system events have
no user row defer optimization, per-side
--system-only-a/--system-only-b on diff-history
defer pairs with deferred per-side --actor-id-a/
--actor-id-b from ADR-0203 Q1, --actor-id null
sentinel rejected for overload concerns. The
retention history surface now has FIVE actor-related
filter dimensions — specific actor positive
(--actor-id from ADR-0186) + specific actor negative
(--actor-id-not from ADR-0206) + system presence
positive (--system-only from this milestone) + system
presence negative (--no-system from this milestone)
+ actor display (--with-actor-names from ADR-0185)
— operators get first-class system-vs-human +
specific-actor inclusion/exclusion audit-log
workflows without LIMIT + pagination + index-usage
correctness problems of jq-filter workarounds. The
`--system-only` / `--no-system` family is now 1-of-3
surfaces: retention history (substrate-side filter
from this milestone); retention diff-history
(expectation check from ADR-0205 Q3 deferred);
retention diff-timeline (substrate-side filter
across all 3 paths from ADR-0207 Q3 + ADR-0193
future Q deferred). Subsequent milestones close the
remaining two surfaces mechanically following the
established per-surface precedent from --actor-id-
not (ADR-0205 expectation check + ADR-0206 history
filter + ADR-0207 diff-timeline multi-value filter).
The retention CLI now has 18 actions with --system-
only / --no-system support shipped on the retention
history surface).
ADR-0210 covers M6.7.zz.tenant.opt-out.cli.history.actor-not.multi
(`crossengin retention history --actor-id-not <uuid>`
repeatable for multi-value OR-semantic exclusion
filter — closes ADR-0206 Q1 by widening the single-
value filter to multi-value via repeated flag using
multiFlags infrastructure from ADR-0183; mirror of
ADR-0207's multi-value pattern on diff-timeline;
closes the within-family asymmetry where retention
history's --actor-id-not stayed single-value while
diff-timeline's went multi-value; real cohort-
exclusion use cases — service-account allowlist audit
(exclude N known SAs in one command), self-exclude
reviewer cohort during incidents (exclude my actor +
on-call + SRE in one command), compliance non-
automation filter (regulatory human review excluding
3-5 automation SAs), cohort negative attestation
(every mutation NOT made by platform-admin cohort);
breaking adapter rename ListOptOutHistoryInput
.actorIdNot?: string → actorIdsNot?: ReadonlyArray
<string> session-recent code from ADR-0206 with no
external consumers contained scope matches ADR-0199
+ ADR-0207 one-shot break precedent; SQL changes
from single-value (h.actor_id IS NULL OR h.actor_id
!= $N) to multi-value (h.actor_id IS NULL OR
h.actor_id NOT IN ($N1, $N2, ...)); single-element
NOT IN ($N) is PG-equivalent to = $N so no
observable behavior change for single-value callers;
empty array treated as filter-not-set matching
ADR-0199/0207 convention operators may legitimately
compute empty list at call site; explicit IS NULL OR
prefix preserved — PG NOT IN returns NULL when
actor_id IS NULL silently filtering system events
OUT (wrong intent); duplicates not deduplicated at
adapter PG handles fine via NOT IN semantic matches
ADR-0207 stance; CLI reads via getMultiFlag instead
of getStringFlag; JSON envelope renamed actorIdNot
→ actorIdsNot string[] | null matching ADR-0207
array-or-null canonical multi-value shape; within-
surface asymmetry documented — retention history's
positive --actor-id stays single-value while negative
--actor-id-not now multi-value documented as future Q
deferred to keep milestone scope focused on closing
ADR-0206 Q1 specifically; asymmetry acceptable
because positive + negative semantically distinct
dimensions (negative path benefits more from multi-
value since "exclude these N noise actors" more
common than "include exactly these N specific
actors"), diff-timeline has both multi-value for
operators wanting multi-value positive, asymmetry
operator-visible via helpText + JSON envelope shape;
adapter test block rewritten + expanded from 8 to 11
tests under renamed describe block — single-element
NOT IN ($1), multi-element NOT IN ($1, $2), omits
clause when not set, empty array filter-not-set
(NEW), includes system events IS NULL prefix
preserved, composes with tenantId multi-actor +
tenant, composes with actorId positive + negative,
composes with joinActor LEFT JOIN + NOT IN,
composes with all filter dimensions full param array,
returns rows excluding listed actors, duplicate
actorIdsNot values produce duplicate placeholders
(NEW) PG dedupes via NOT IN; plus one cross-reference
fix in system-only describe block; CLI test block
rewritten + expanded from 6 to 8 tests under renamed
describe block — threads single-element array when
set once, threads multi-element array when repeated
(NEW), omits when not set backward compat, composes
with --actor-id positive + negative both threaded,
composes with --tenant + --with-actor-names + multi
--actor-id-not, JSON envelope echoes single-element
array, JSON envelope echoes multi-element array
(NEW), JSON envelope actorIdsNot=null when not set;
test count 8,955 → 8,960 (+5 net: adapter +3, CLI
+2); cli.ts helpText updated from [--actor-id-not
<uuid>] to [--actor-id-not <uuid> ...] indicating
repeatable + description extended explaining
repeatable + OR-semantic NOT IN + system-events-
included semantic; rejected alternatives — two-field
adapter surface (actorIdNot string + actorIdsNot
array permanent two-field invites operator confusion;
one-shot rename cleaner matches ADR-0199),
--actor-id-not <a,b,c> comma-separated (fragile with
shell-quoted UUIDs containing punctuation),
--actor-id-not-list <file.txt> (adds file-reading
code path defer), keep single-value document jq-
workaround (fails LIMIT + pagination correctness),
widen --actor-id to multi-value in same milestone
(would close ADR-0186 future Q + restore within-
surface symmetry BUT exceeds requested scope
documented as future Q), substrate-side dedup of
duplicates (PG handles fine), CLI-side UUID
validation per occurrence (PG errors clearer matches
deferred decision), pipe-separated alternative
(shell-quoting concerns), ENV-style flag names
(verbose), adapter-side empty-array rejection (
operators may legitimately compute empty list
operator-friendly). The --actor-id-not family is now
multi-value uniformly across both filter surfaces —
retention history (this milestone) + retention diff-
timeline (ADR-0207) — plus single-value expectation
check on the cross-event diff surface (retention
diff-history from ADR-0205); operators get cohort-
exclusion ergonomics with surface-appropriate
semantics under one consistent flag name. The
retention CLI now has 18 actions with multi-value
...
--actor-id-not support on the two list-style audit-
log filter surfaces.
ADR-0212 covers M6.7.zz.tenant.opt-out.cli.diff-history.system-only
(`crossengin retention diff-history --system-only` /
`--no-system` actor-presence expectation check — closes
ADR-0205 Q3; diff-history expectation-check companion to
ADR-0209's substrate-side filter on retention history;
--system-only asserts BOTH events have actor_id IS NULL
(both system-authored); --no-system asserts NEITHER has
actor_id IS NULL (both have UUID actor); semantic differs
from ADR-0209's filter because diff-history takes 2 fixed
IDs not a list query — flag shifts from filter to
expectation check matching ADR-0198/0203/0205/0208 pattern
where the same flag becomes per-event-pair assertion;
mutually exclusive at CLI boundary both set returns exit
2 with explicit error before any PG query matching
ADR-0209 history pattern; composes with all existing
expectation flags --kind --kind-not --actor-id
--actor-id-not --with-actor-names without restriction;
reuses ActorPresenceFilter discriminated union from
ADR-0209 same type same mutual-exclusivity same JSON
envelope echo shape; DiffHistoryEntriesInput gains
optional actorPresence?: ActorPresenceFilter field; new
expectation check block positioned after existing
actorIdNot check (actor-dimension group); system_only
throws naming offending side(s) with actual actor_id
value ("but A is '<uuid>'") mirroring ADR-0203 positive
--actor-id rendering — operators need to see WHO authored
the event if it wasn't supposed to have a human actor;
no_system throws naming side(s) as <system> placeholder
("but A is <system>" / "both A and B are <system>")
mirroring ADR-0205 negative --actor-id-not rendering —
operators already know which side they expected to be
human naming the sentinel suffices; different rendering
reflects different information needs (positive needs to
show unexpected UUID; negative just names the side since
<system> is the excluded value); check ordering —
eventKind → eventKindNot → actorId → actorIdNot →
actorPresence (kind-dimension first actor-dimension
second with specific-actor before presence); use cases
unblocked — system-only audit pair verification (assert
both events are scheduled sweep events), human-only
forensic comparison (assert neither is system before
drawing responsibility conclusions), compliance
attestation gating (--no-system + --kind opt_out_set),
migration verification (--system-only + --kind
retention_set); CLI threads systemOnlyFlag + noSystemFlag
via getBooleanFlag, mutual-exclusivity check, translate
to actorPresence single value, thread to adapter, echo
systemOnly + noSystem booleans in JSON envelope matching
ADR-0209 history boolean-echo convention; composition
with --actor-id / --actor-id-not — --system-only +
--actor-id contradictory at data layer (first check
fires natural first-fail ordering), --system-only +
--actor-id-not redundant but valid, --no-system +
--actor-id canonical "both must be human X" idiom,
--no-system + --actor-id-not canonical "both must be
human excluding X" idiom; why expectation check not
filter — diff-history takes 2 fixed IDs PG-side WHERE
would silently return zero rows on mismatch
indistinguishable from "IDs don't exist" adapter throw
with clear message naming offending side(s) is loud and
clear matches ADR-0198/0203/0205/0208 precedent;
mismatch exits 1 (runtime path) CLI misuse mutual-
exclusivity exits 2; 10 new adapter tests in a new
"PostgresTraceRetention.diffHistoryEntries actorPresence
expectation check" describe block — system_only accepts
when both null, throws naming A when A has UUID with
actual UUID in error, throws naming B when B has UUID,
throws naming both sides when neither is system,
no_system accepts when both have UUID actor, throws when
A is <system> explicit rendering, throws when B is
<system>, throws naming both when both are <system>,
omits check when actorPresence not set backward compat,
composes with --kind expectation check (both pass when
actor + kind expectations met); 9 new CLI tests in a
new "runRetention diff-history --system-only /
--no-system" describe block — exit 2 when both flags
set with explicit error, threads actorPresence='system_
only' when --system-only set, threads actorPresence=
'no_system' when --no-system set, omits actorPresence
when neither flag set backward compat, composes with
--actor-id-not + --no-system both threaded
independently, adapter expectation error propagates exit
1 with explicit error including actual UUID, JSON
envelope echoes systemOnly=true + noSystem=false when
--system-only set, JSON envelope echoes noSystem=true +
systemOnly=false when --no-system set, JSON envelope
both false when neither set; test count 8,966 → 8,985
(+19: adapter +10, CLI +9); cli.ts helpText extended
for retention diff-history usage line with [--system-
only | --no-system] notation indicating mutual
exclusivity + 4-line description added explaining
expectation-check semantic + mutual exclusivity;
rejected alternatives — two separate boolean fields on
adapter (allows invalid both-true state; discriminated
union makes invalid unrepresentable), --actor-presence
single flag with enum value (operators remember enum
vocab vs natural boolean flags), --system/--human flag
names ("human" misleading SAs not human), --actor-id
null sentinel (overloads UUID semantic), CLI-side
--actor-id + --system-only contradiction error (would
block valid edge cases natural first-check-fires
outcome), substrate-side both-flag mutual-exclusivity
in adapter (type union makes invalid unrepresentable),
per-side --system-only-a/--system-only-b (overkill v1
defer), render mismatch as fieldDiff (actor presence
isn't field in next_state JSONB throwing loud and
clear), PG WHERE clause validation (would silently
return zero rows indistinguishable from missing IDs),
default --no-system silent behavior change backward-
incompatible. The retention diff-history surface now
has FIVE expectation-check flag families forming a 3×2
+ 1 matrix — kind dimension (--kind ADR-0198 +
--kind-not ADR-0208) + specific actor_id dimension
(--actor-id ADR-0203 + --actor-id-not ADR-0205) +
system actor presence dimension (--system-only +
--no-system from this milestone) + actor display
(--with-actor-names ADR-0204) — operators get full
positive + negative expectation gates across kind +
specific-actor + system-presence dimensions with
consistent error-message shape (positive: "but A is
'Y'"; negative: "but A matches" / "but A is <system>")
and uniform exit-code semantics (CLI misuse exit 2 on
mutual exclusivity; runtime exit 1 on adapter-level
expectation violation). The --system-only / --no-system
family is now 2-of-3 surfaces shipped: retention
history (substrate-side filter from ADR-0209); retention
diff-history (expectation check from this milestone);
retention diff-timeline (substrate-side filter across
all 3 paths from ADR-0207 Q3 + ADR-0193 future Q
...
ADR-0214 covers M6.7.zz.tenant.opt-out.cli.diff-history.kind-not.multi
(`crossengin retention diff-history --kind-not` repeatable
for multi-value OR-semantic exclusion expectation check —
closes ADR-0208 Q1; widens single-value --kind-not to
multi-value via repeated flag using multiFlags from
ADR-0183; operator declares "I expect NEITHER event to
have any of these N event_kinds" and adapter throws on
match with side(s) named; mirror of ADR-0210 history
actor-id-not multi-value + ADR-0207 diff-timeline actor-
id-not multi-value + ADR-0200 diff-timeline multi-kind —
same flag-repetition pattern + same one-shot breaking
rename + same per-occurrence validation against 4-value
enum + same empty-array-as-filter-not-set convention;
real cohort-exclusion use cases — workflow audit
excluding maintenance kinds (assert NEITHER event is
policy_deleted OR retention_set), anti-automation pair
verification, CI gate with kind-list exclusion, cohort-
aware forensic comparison composing with --system-only
from ADR-0212; breaking adapter rename
DiffHistoryEntriesInput.eventKindNot?:
OptOutHistoryEventKind → eventKindsNot?: ReadonlyArray
<OptOutHistoryEventKind> session-recent code from
ADR-0208 with no external consumers contained scope
matches ADR-0199/0207/0210/0211 one-shot break
precedent (6 consecutive single→multi renames make this
mechanical); adapter rewritten with Set-based O(1)
membership lookup + always-list error format
`event_kind in ['X']` for single-value + `event_kind
in ['X', 'Y']` for multi-value (single-value error
breaks from ADR-0208's `event_kind 'X'` shape
acceptable since field rename is itself breaking;
consistent rendering across single + multi reads
better than separate special-case formats); CLI reads
via getMultiFlag instead of getStringFlag with per-
occurrence validation via isOptOutHistoryEventKind
loop matching ADR-0200 pattern — exits 2 on FIRST
invalid value with that value named (operators reading
error know exactly which --kind-not occurrence
failed); JSON envelope renamed kindNot: string | null
→ kindsNot: string[] | null matching ADR-0207/0210/
0211 array-or-null canonical multi-value envelope;
within-action asymmetry documented — positive --kind
stays single-value while negative --kind-not now
multi-value; documented as future Q (widen --kind to
multi-value tuple expectation closes ADR-0198 Q2 +
restores within-action symmetry); deferred to keep
milestone scope focused on closing ADR-0208 Q1
specifically; asymmetry acceptable because positive +
negative semantically distinct dimensions (negative
path benefits more from multi-value since "exclude
these N noise kinds" is more common than "include
exactly one of these N specific kinds"), the 4-value
enum makes positive less compelling (operators wanting
"any of these 3 kinds" can equivalently use "neither
of remaining 1 kind" via --kind-not — negative is
strict superset); adapter test block rewritten +
expanded from 8 to 12 tests including new multi-value
accept + multi-value throws with multi-value error
format + empty-array filter-not-set + new error format
single-value coverage; CLI test block rewritten +
expanded from 7 to 10 tests including new FIRST-
invalid-occurrence exit 2 + new multi-element threading
+ new multi-element JSON envelope echo; test count
9,004 → 9,011 (+7 net: adapter +4, CLI +3); cli.ts
helpText updated from [--kind-not <event-kind>] to
[--kind-not <event-kind> ...] indicating repeatable +
description extended explaining repeatable + NEITHER
event has any of listed kinds + match-by-either-side
semantic; rejected alternatives — two-field adapter
(permanent invites confusion one-shot rename cleaner),
--kind-not <a>|<b>|<c> pipe-separated (fragile shell-
quoting), --kind-not <a>,<b>,<c> comma-separated
(fragile defer), keep single-value document jq-
workaround (fails exit-code-semantic argument), ship
in same milestone as --kind multi-value tuple (exceeds
user scope documented as future Q), keep single-value
error message format (inconsistent across single/
multi-value; always-list predictable), PG-side WHERE
NOT IN (diff-history takes 2 fixed IDs not list query),
substrate-side dedup (Set handles fine matches
ADR-0207/0210/0211), auto-validate at adapter (CLI
handles validation at boundary), separate --kind-not-
any flag (adds CLI surface with two near-identical
names); future Qs — widen --kind to multi-value tuple
expectation closes ADR-0198 Q2 + restores within-
action symmetry natural follow-up, --kind-not @file
defer since 4-value enum bounded, --kind-not filter
on retention history pairs with ADR-0206 Q4 defer,
--kind-not filter on diff-timeline across all 3 paths
pairs with ADR-0207 Q4 defer, CLI-side dedup defer,
semantic-shape exclusion --exclude-write-mutations
shorthand operator-policy defer. The retention diff-
history surface uniquely keeps single-value semantics
on most flags because expectation-check has different
operator ergonomics than list-query filter — operators
making per-event assertions usually have specific
values in mind not cohorts. The negative kind-not
filter is the first multi-value expectation check on
diff-history; the symmetric positive --kind multi-
value is the natural follow-up. The retention CLI now
has 18 actions with comprehensive multi-flag coverage
spanning the actor + kind dimensions across all 3
surfaces with surface-appropriate semantics).
ADR-0215 covers M6.7.zz.tenant.opt-out.cli.diff-history.per-side
(per-side asymmetric expectation checks on retention
diff-history — closes ADR-0198 Q1 + ADR-0203 Q1 +
ADR-0205 Q2 + ADR-0208 Q2 covering 4 of the 5 per-side
future Qs in one milestone; 5th future Q ADR-0212 Q2
--system-only-a/--no-system-a per-side boolean-pair
deferred since the 4-flag intra-side mutual-exclusivity
design space needs its own treatment; ships 8 new per-
side flags paired with their existing global
counterparts — --kind-a/--kind-b closes ADR-0198 Q1,
--kind-not-a/--kind-not-b multi-value following
ADR-0214 closes ADR-0208 Q2, --actor-id-a/--actor-id-b
closes ADR-0203 Q1, --actor-id-not-a/--actor-id-not-b
closes ADR-0205 Q2; ADDITIVE not breaking rename
DiffHistoryEntriesInput gains 8 new optional fields
preserving the existing 5 global fields per-side flags
are semantically DIFFERENT operations from global flags
not just renames; check ordering global before per-side
within each pair (global symmetric fires first when
combined with per-side asymmetric); per-side error
messages use explicit "event A" / "event B" prefix
with positive showing actual unexpected value mirroring
ADR-0203 + negative naming side without showing value
mirroring ADR-0205; <system> placeholder for null
actor_id preserved across per-side; CLI parsing 8 new
flag parses with appropriate validation matching
existing global counterparts; JSON envelope gains 8
new fields matching CLI flag names; composition with
global flags fires global first natural ordering;
real asymmetric use cases — forensic state-transition
verification A=deletion B=rebuild, per-side actor
attribution A=Alice B=Bob, per-side exclusion different
actors per side, per-side multi-value kind exclusion,
hybrid global + per-side belt-and-suspenders,
maximum-discipline forensic assertion combining all
per-side variants; 17 new adapter tests + 17 new CLI
tests = 34 total; test count 9,011 → 9,045; rejected
alternatives — combine into global flags via A/B
comma-separated semantic-overload breaking change,
positional A/B semantics unusual flag form, substrate-
side dedup of contradictory global + per-side
operator may script CLI doesn't pre-empt, ship --system-
only-a/-b in this milestone boolean-pair × per-side
deserves own design defer, --actor-id-not-a vs
--actor-id-a-not naming consistency adopted, single
combined --side modifier flag verbose + non-composable,
--a-kind/--b-kind prefix form non-canonical, per-side
multi-value --actor-id-a repeatable separate design
defer, adapter-side global + per-side mutual exclusivity
check operator intent ambiguous CLI passes both
adapter runs both, Map-based adapter input verbose
flat fields match CLI flag names; future Qs — --system-
only-a/-b + --no-system-a/-b boolean-pair × per-side
defer closes ADR-0212 Q2, per-side multi-value
--actor-id-a repeatable for tuple expectation defer,
JSON envelope field name distinction between operator-
input vs result diff values defer rename, apply per-
side to retention diff-timeline N/A list-style not per-
event-pair, --kind-a accepts multi-value tuple defer,
combined per-side + system actor presence --system-
only-a + --no-system-b defer. The retention diff-
history surface now has 13 expectation-check flag
families forming a 4×3 matrix — 4 dimensions
(event_kind positive, event_kind negative, actor_id
positive, actor_id negative) × 3 modes (global
symmetric, per-side A, per-side B) plus the actor-
presence pair --system-only/--no-system (global only);
operators have unprecedented control over per-event-
pair forensic assertions with symmetric + asymmetric
expectations across kind + actor + system-presence
dimensions in a single command. The retention CLI now
has 18 actions with the most comprehensive multi-flag
coverage in the codebase — operators can express
virtually any state-transition assertion in a single
command. The natural follow-up milestones — --system-
only-a/-b + --no-system-a/-b per-side boolean-pair
(ADR-0212 Q2), multi-value tuple expectations on per-
side variants (ADR-0198 Q2 / ADR-0203 Q2 family)).
ADR-0216 covers M6.7.zz.tenant.opt-out.cli.diff-history.per-side.system-only
(per-side --system-only/--no-system actor-presence
expectation check on retention diff-history — closes
ADR-0212 Q2 + ADR-0215 deferred 5th per-side future
Q; ships 4 new boolean flags (--system-only-a,
--no-system-a, --system-only-b, --no-system-b)
completing the per-side expectation-check matrix on
diff-history; the 4-flag intra-side mutual-exclusivity
design space resolves cleanly by reusing the
ActorPresenceFilter discriminated string union from
ADR-0209/0212 per side — same type at adapter level,
CLI handles 4 booleans + intra-side mutual exclusivity
check + translates to 2 union values; ADDITIVE
DiffHistoryEntriesInput gains actorPresenceA/B?:
ActorPresenceFilter preserving global actorPresence;
adapter check blocks positioned at end of actor
dimension chain (after global actorPresence); per-
side error format mirrors ADR-0215 asymmetric
rendering (positive shows actual UUID, negative
renders <system> placeholder); CLI intra-side mutual
exclusivity checks fire BEFORE adapter call;
composition with global — global fires first natural
ordering; canonical asymmetric --system-only-a +
--no-system-b for "A is system, B is human"; 10 new
adapter tests + 13 new CLI tests = 23 total; test
count 9,045 → 9,068; rejected — two boolean field
pairs on adapter (allows invalid both-true per side
discriminated union cleaner), single combined
actorPresence 4-value union (combinatorial
explosion), --side-A-presence parameterized flag
unusual form, CLI-side global + per-side contradiction
error (operator intent ambiguous), positional A B
arguments unusual form, adapter-side mutual exclusivity
(operator intent ambiguous); future Qs — per-side
--system-only-X + per-side --actor-id-X contradiction
detection defer, JSON envelope unification defer
breaking, apply per-side to retention diff-timeline
+ retention history N/A list-style, short-circuit
LEFT JOIN when actorPresenceA='system_only' defer
optimization, combined actor-presence + actor-id
consistency defer. The retention diff-history surface
now has the COMPLETE per-side expectation-check
matrix — 5 dimensions × 3 modes (global symmetric +
per-side A + per-side B) = 15 expectation-check flag
families plus --with-actor-names; the matrix is now
fully symmetric every dimension has both global and
per-side variants. Natural follow-up milestones —
multi-value tuple expectations on actor-id positive/
negative (ADR-0203 Q2 + ADR-0205 Q1), multi-value
tuple expectation on global --kind positive (ADR-0198
Q2), per-side multi-value variants combining ADR-0215
per-side pattern with future multi-value tuples. The
retention CLI now has 18 actions with the most
comprehensive multi-flag coverage in the codebase;
the per-side asymmetric expectation-check matrix on
diff-history is COMPLETE for all 5 actor + kind +
system-presence dimensions).
...
ADR-0186 covers Phase 2 M6.7.zz.tenant.opt-out.cli.history.actor-filter
(`crossengin retention history --actor-id <uuid>`
filter — closes ADR-0185 Q1; after ADR-0170 shipped 5
filter dimensions (tenant, table, kind, since, until),
ADR-0175 added --after-id cursor pagination, and
ADR-0185 added --with-actor-names display surfacing,
operators kept wanting "show me everything actor X
did in the last week" — current jq-filter workaround
broke LIMIT correctness (asking for --limit 100
returned <100 entries after post-fetch filter) and
pagination correctness (--after-id semantics with
jq-side filter are wrong; skip-then-filter is wrong,
filter-then-skip is right; substrate-side filtering
is the only way to get this right) plus index usage
(PG can use composite index on (actor_id,
occurred_at) if added; jq filtering can't); single
substrate-side WHERE filter eliminates jq dance.
CLI surface `retention history [...other flags...]
[--actor-id <uuid>]` — string flag scoping history to
mutations performed by that actor; when omitted no
actor filter applied existing behavior preserved.
Adapter — ListOptOutHistoryInput gains optional
actorId?: string field; when set SQL WHERE adds
h.actor_id = $N condition with index-scan on existing
actor_id column; filter position between eventKind
and since in WHERE-clause assembly order placing
actor-based filters near "who" semantically (tenant +
table + kind + actor + time-range); param-number
sequence $1: tenantId, $2: tableName, $3: eventKind,
$4: actorId, $5: since, $6: until, $7: afterId, $8:
limit when all set. Substrate-side not CLI-side — the
filter happens in PG via WHERE clause not via post-
fetch jq; three benefits: LIMIT correctness
(operators asking for --limit 100 --actor-id <x> get
the latest 100 entries BY that actor not the latest
100 entries filtered to that actor which may return
<100), cursor pagination correctness (--after-id
semantics with --actor-id work intuitively — skip-
then-filter-by-actor is wrong; filter-then-skip is
right; substrate-side filtering is the only way),
index usage (PG can use composite index on
(actor_id, occurred_at) if added; jq filtering can't).
Composes uniformly with every existing filter
dimension — --tenant + --actor-id (actor's mutations
on one tenant), --table + --actor-id (one table),
--kind + --actor-id (specific event kind), --since /
--until + --actor-id (time range), --after-id +
--actor-id (paginated actor-scoped history),
--with-actor-names + --actor-id (actor-scoped +
display-name surfacing — canonical "Alice's audit
log" pattern). No null-actor filtering — operators
wanting "only system-actor events" where actor_id IS
NULL jq-filter on .actorId === null; three reasons:
substrate stays minimal (one filter per dimension no
sentinel values), null is logically the absence of an
actor not a specific actor ID, jq idiom is one-liner
(crossengin retention history --format json | jq
'.entries[] | select(.actorId == null)'); future Q if
measured demand. JSON envelope gains an actorId field
(string when --actor-id set null when not) operators
piping through jq can branch on it; position between
eventKind and since matches WHERE-clause order. Pure
CLI delivery modulo adapter input field — the adapter
gets one new field (actorId?: string), the CLI gets
one new flag (--actor-id), no new types no new
helpers no result-shape changes mechanical addition.
Use cases unblocked — per-actor incident response
(crossengin retention history --actor-id <suspect-
actor-uuid> --with-actor-names --since 2026-05-01
lists every mutation by that actor since May 1 with
display name), compliance attestation per officer
(for officer in $(cat compliance-officers.txt); do
crossengin retention history --actor-id "$officer"
--tenant "$regulated-tenant" --format json; done |
jq -s aggregating), "what did Alice do in the last
7 days?" (--actor-id alice_uuid --with-actor-names
--since 7-days-ago reads like English; output is
clean per-actor changelog), CI gate on unauthorized-
actor mutations (crossengin retention history
--actor-id service-account-id --since build-start
--format json | jq '.entries | length' — if > 0
automated mutation occurred during build window
investigate). Rejected alternatives — CLI-side jq
filtering as documented workflow (rejected substrate-
side ensures LIMIT + pagination correctness jq-side
returns < N entries when filtering bites),
--actor-id-equals / --actor-id-not pair (overkill
for v1 positive-only is common case rejected defer
not-equals variant), multi-actor --actor-id repeated
via multiFlags (operators wanting OR-semantic across
N actors pair-wise sufficient for now run twice +
concat multi-actor defer), --actor system sentinel
for null filtering (overloads string semantic null
is valid filter doesn't need sentinel operators jq-
filter), add actor_id index in this milestone
(schema migration concern defer until measured slow),
--actor-id-pattern regex for pattern matching
(operators with structured ID schemes uncommon
defer), implicit actor scoping via --my or
$CROSSENGIN_ACTOR_ID env var (operator-state-
dependent magic explicit --actor-id clearer),
surface in JSON envelope as actorFilter not actorId
for naming consistency with tenantFilter/tableFilter
(chose actorId because per-entry actorId field
already uses that name envelope-level matches
entry-level rejected verbose form). Drawbacks — no
null-actor sentinel operators wanting actor_id IS
NULL filtering jq-filter (documented), no
--actor-id-not exclusion operators excluding one
actor (everything Alice didn't do) jq-filter defer,
no multi-actor filter (--actor-id alice --actor-id
bob with OR semantics; operators run two commands +
concatenate could use multiFlags from ADR-0183 —
defer), no --actor-name-equals for human-readable
input (requires actor → UUID resolution operators
look up UUIDs first; future Q paired with ADR-0185
Q2), substrate-side index requirement actor_id
filtering without composite index (actor_id,
occurred_at) does sequence scan + sort defer until
measured slow meta-schema currently has no actor_id
index on tenant_retention_opt_out_history, no
validation of UUID shape at CLI boundary operators
passing invalid UUIDs hit PG's error message rather
than a CLI exit-2 (matches existing pattern from
--after-id ADR-0175 which deferred validation to PG).
7 new adapter tests in trace-retention.test.ts —
adds h.actor_id = $N WHERE clause when actorId is
set verified via SQL substring + params [ACTOR_A,
100] (limit at end), omits actor_id WHERE clause
when actorId is not set verified via not.toContain,
composes with other filters tenantId + actorId
(h.tenant_id = $1 + h.actor_id = $2 verified with
params [TENANT_A, ACTOR_A, 100]), composes with
joinActor actor-id filter + LEFT JOIN names (LEFT
JOIN meta.users + h.actor_id = $1 both present),
composes with all filter dimensions tenant + table
+ kind + actor + since + until + afterId + limit
(full param array [TENANT_A, workflow_traces,
opt_out_set, ACTOR_A, since, until, afterId, 50]
verified plus all 7 SQL substrings present),
returns only rows matching the actor verified via
mock row + adapter result actorId check, returns
empty array when filtered actor has no rows. 7 new
CLI tests in retention.test.ts — threads actor-id
to adapter when --actor-id is set verified via
historyCapture, omits actorId from adapter input
when --actor-id NOT set backward compat verified,
composes with other filters --tenant + --actor-id
+ --kind (full input shape matched), composes with
--with-actor-names actor-id filter + LEFT JOIN
names (capture verifies both actorId + joinActor:
true threaded), JSON envelope includes actorId
field with value when set, JSON envelope actorId
is null when --actor-id not set, human-format
empty-result message preserved when --actor-id has
no matches ("no history entries match"). cli.ts
helpText extended with --actor-id <uuid> flag note
added to retention history usage line + description
mentioning "actor filtering for one actor's
mutations" + 6-filter-dimension semantic. Future Qs
cover --actor-name-equals <name> filter resolving
through meta.users.display_name (requires JOIN
pairs with ADR-0185 --with-actor-names
infrastructure future Q), multi-actor via repeated
--actor-id using ADR-0183 multiFlags infrastructure
(operators with cohort audit needs would benefit
defer until measured demand), --actor-id-not <uuid>
for exclusion defer, composite index on (actor_id,
occurred_at) for large-scale actor-scoped
pagination defer until measured slow, CLI-side UUID
validation matching ADR-0175 deferred decision PG
gives clearer error, --system-only boolean flag for
actor_id IS NULL filtering defer operators jq-filter
substrate doesn't need sentinel, surface --actor-id
filter in audit-log diff actions like retention
diff-history --actor-id <uuid> filtering events by
actor before diffing different surface future ADR
if requested. The retention CLI now has 17 actions
with 6 filter dimensions on retention history
(tenant + table + kind + actor + since + until)
plus cursor pagination + --with-actor-names display
...
ADR-0191 covers Phase 2 M6.7.zz.tenant.opt-out.cli.diff-timeline.add-tenant
(`crossengin retention diff-timeline --add-tenant <c>
[--add-tenant <d> ...]` repeated flag for N-way
cross-tenant chronological merge — closes ADR-0189 Q1;
ADR-0189 shipped pair-wise timeline merge for 2
tenants but operators investigating cohort-wide
divergence ("across 5 regulated tenants, who diverged
from the reference policy first and when?") had to
run N pair-wise commands and manually merge by
timestamp; this milestone mirrors ADR-0183's
--add-tenant pattern on the timeline axis; CLI surface
`retention diff-timeline <a> <b> <table> [--add-tenant
<c> ...] [--since DATE] [--until DATE] [--limit N]
[--with-actor-names]` base call still requires 2
positional tenants + 1 positional table matching
ADR-0189; --add-tenant repeatable via multiFlags from
ADR-0183; N total = 2 + count(--add-tenant) minimum
N for N-way mode = 3; all other flags (--since,
--until, --limit, --with-actor-names) compose
uniformly across both paths; adapter — new
diffHistoryTimelineNway separate from
diffHistoryTimeline mirroring ADR-0183 separate-method
pattern; takes {tenantIds: ReadonlyArray<string>,
tableName, since?, until?, limit?, joinActor?};
returns {tenantIds, tableName, entries: NwayTimelineEntry[]}
where NwayTimelineEntry has tenantLabel: string field
(widened from pair-wise tenantSide: "A" | "B"); why
separate adapter method — narrows TypeScript types
for pair-wise consumers (tenantSide literal union vs
N-way widened-string tenantLabel), preserves backward
compat, JSON envelope discriminator nway:true clearly
distinguishes response shapes; dispatcher in
runRetentionDiffTimeline reads --add-tenant via
getMultiFlag, branches to N-way path when set, falls
through to existing pair-wise path otherwise; SQL —
single query with `h.tenant_id IN ($1, $2, $3, ...)`
PG handles efficiently with existing (table_name,
occurred_at) index; two parallel queries per tenant
would be O(N) round-trips + careful tie handling
under shared occurred_at; per-row tenantLabel tagged
at adapter via Map<tenantId, label> built from input
order — duplicate tenantIds first occurrence wins
the label matches ADR-0183 dedup pattern; new
labelForIndex exported helper assigns 'A'..'Z' for
i<26 then 'T27'/'T28'/... matching ADR-0183/0188
cohort label convention beyond 26 tenants; human
output renders 'N-way timeline for N tenants on
<table>:' header + per-tenant 'Tenant A: <uuid>' rows
+ 'Events (N):' header + per-event '<occurred_at>
[<label>] <event-kind-padded-16> <state-summary>
[by <actor>]' lines; empty-events case renders 'No
history events for any of these tenants on this
table.'; new formatTimelineNwayDiff exported helper
reuses existing summarizeTimelineEntry +
formatActor helpers from ADR-0189/0190 — no
duplication; JSON envelope {action: "diff-timeline",
nway: true, since, until, limit, withActorNames,
result: DiffHistoryTimelineNwayResult} where nway:
true is the discriminator; operators jq-filter on
.result.entries[] | select(.tenantLabel == "C") to
extract one tenant's events from merged cohort
timeline; use cases unblocked — cohort-wide
divergence audit (5-tenant cohort in one command +
jq group_by per tenant), legal-hold cohort
verification across N hold-tenants in single command,
tier migration drift detection filtering on
tenantLabel via jq, compliance attestation across
regulated cohort × prunable tables via shell loop;
rejected alternatives — widen diffHistoryTimeline to
ReadonlyArray<string> of length 2..N (loses pair-wise
TypeScript narrowing tenantSide literal becomes
widened string), N-way via --tenants <a,b,c> comma-
separated single flag (fragile to embedded commas
operators with variables containing commas hit edge
cases), auto-promote to N-way when >3 positional
args (magical), new action retention diff-timeline-
nway (adds CLI surface flag-on-existing matches
--add-tenant precedent), use tenantSide field on
N-way entries widened to string (less honest about
cross-axis semantic tenantLabel matches ADR-0188
labels rename), JSON envelope without nway:true
discriminator (operators detect by tenantIds[] vs
tenantIdA/tenantIdB shape — explicit discriminator
clearer ADR-0183/0180/0188 precedent), N-way +
--vs-platform synthetic comparison different
semantic defer, state-replay per-event divergence
delta against cohort significant query cost defer,
--exclude-tenant set-subtraction defer; drawbacks —
two adapter methods to maintain
(diffHistoryTimeline + diffHistoryTimelineNway
mirrors ADR-0183; pair-wise narrow TypeScript types
preserved for common case), tenantSide literal union
vs tenantLabel widened string field name difference
(callers branch on nway discriminator first
acceptable), label collision at >26 tenants T27/T28
less readable than alphabet but substrate handles
documented, no input tenantIds deduplication
duplicates survive in tenantIds[] but first
occurrence wins label and SQL de facto dedups via
IN-clause matches ADR-0183 behavior, no N-way ×
--cross-table matrix (different semantic axis
operators run multiple commands defer), single IN-
clause query PG handles tens of tenants fine but
thousands hit parser limit (defer chunking until
measured slow), no state-replay (operators wrap
with `retention diff --add-tenant` for current-state
divergence); 11 new adapter tests in trace-
retention.test.ts (rejects fewer than 2 tenantIds
with 'at least 2 tenantIds required' error, issues
single query with h.tenant_id IN ($1, $2, $3) for
3 tenants verified via SQL substring + params, tags
each entry with tenantLabel A/B/C from input order,
assigns T27/T28 labels for indices beyond 26,
...
positions after tenant IN list (4 tenants → since
at $6 until at $7), composes with joinActor=true
emitting LEFT JOIN meta.users + actor cols, rejects
limit < 1 with explicit error, throws on unknown
event_kind defensive schema-drift guard, preserves
duplicate tenantIds first-occurrence wins the
label); 8 new CLI tests in retention.test.ts —
dispatches to N-way path when --add-tenant is
present (capture verifies tenantIds + tableName
threaded), collects multiple --add-tenant flags in
argv order [A, B, C, D] via repeated occurrences,
does NOT call diffHistoryTimelineNway when
--add-tenant absent backward compat for pair-wise
path verified, threads --with-actor-names + --since
+ --limit through to N-way adapter, human-format
'N-way timeline for 3 tenants on workflow_traces'
header + per-tenant rows, human-format renders
[A]/[B]/[C] tagged events with '(policy deleted)'
for null nextState, JSON envelope nway:true
discriminator + result.tenantIds + entries with
tenantLabel, adapter errors on N-way path propagate
exit 1; 3 new formatTimelineNwayDiff helper unit
tests — 'No history events for any of these tenants'
empty rendering, [A]/[B]/[C] tagged event lines with
state summary including retention/opt_out/reason,
'by Alice Smith (uuid)' suffix with withActorNames=
true opt; fakeRetention extended with
diffHistoryTimelineNway mock + diffTimelineNwayResult
+ diffTimelineNwayCapture options; cli.ts helpText
extended with --add-tenant flag note + 3-line
description explaining N-way semantic + matches
[A]/[B]/... tagging + composes with all other flags;
future Qs cover N-way + --cross-table matrix mode
(N tenants × M tables merged timeline output
dimensions multiply defer), N-way + --vs-platform
synthetic comparison defer, --exclude-tenant for
set-subtraction defer, --add-tenant <slug> resolving
via meta.tenants.slug for human-readable input
pairs with similar deferred Qs across ADRs defer,
cursor pagination via --after-id on N-way matching
ADR-0175 same pattern defer, state-replay variant
emitting per-event divergence delta against the
cohort defer, tagged-union JSON envelope across all
retention diff-* actions pairing with similar
deferred Qs across ADR-0173/0178/0179/0180/0183/
0188/0189/0190 defer, auto-deduplicate tenantIds at
CLI boundary operators wrap with sort -u on input
defer. The retention CLI now has 18 actions with
N-way cohort comparison spanning both temporal
axes — point-in-time diff via retention diff
--add-tenant from ADR-0183 + chronological diff via
retention diff-timeline --add-tenant from this

...
P1.27 + P1.28 + P2.18 + P2.19 + P2.20 + P2.21 + P2.22 + P2.23 + P2.24 + P2.25 + P2.26** landed: **61 packages + 3 apps, 124
meta-schema tables, 6,600 offline tests + 27 gated real-Postgres
integration tests (16 worker + 11 operate-server) + a CI incident-drift
gate**, all green, no type errors. (Earlier
...
- **`apps/workflow-worker`** — Phase 3 P2.3: the runnable
  ...
  acked_at/mitigated_at — so metrics also report MTTA/MTTM; P2.25/ADR-0134
...
suites under `CROSSENGIN_PG_TEST=1`, then an **incident-drift gate** —
`workflow-worker incidents verify` over a wide window, which exits 1 and
fails the build on any drifted incident timeline). `scripts/emit-bootstrap.mjs`

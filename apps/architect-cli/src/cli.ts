...
export function helpText(): string {
  return [
    ...
    "                          Bob's mutations'). With --kind: filter to one event_kind",
    "                          (opt_out_set|opt_out_cleared|retention_set|policy_deleted).",
    "                          With --after-id: cursor forward in time (newer entries since",
    "                          diff-timeline is ASC). With --before-id: cursor backward toward",
    "                          older entries. --after-id and --before-id are mutually",
    "                          exclusive. Output gives nextAfterId + nextBeforeId when page",
    "                          is full. (requires PG env)",
    ...
    "                          --actor-id repeatable for OR-semantic multi-actor filter.",
    "                          --kind filters to one event_kind. --after-id cursors forward,",

...
export function helpText(): string {
  return [
    ...
    "                          [--with-actor-names]",
    "                          Show how two tenants' retention policies evolved over time on",
    "                          one table. Merges history events from both tenants into a",
    "                          single chronological timeline (ASC). Each event tagged with",
    "                          [A] or [B] indicating which tenant changed. With",
    "                          --with-actor-names: LEFT JOIN meta.users + render",
    "                          'display_name (uuid)' alongside each event. (requires PG env)",

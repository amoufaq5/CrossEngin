...
export function helpText(): string {
  return [
    ...
    "                    [--limit N] [--after-id <uuid>] [--before-id <uuid>]",
    "                    [--with-actor-names]",
    ...
    "                          to one actor's mutations. --with-actor-names LEFT JOINs",
    "                          meta.users to surface actor display_name + email alongside",
    "                          raw UUIDs. (requires PG env)",
    ...
    "                          'display_name (uuid)' alongside each event. With --actor-id:",
    "                          filter to events authored by that actor. With --kind: filter to",
    "                          one event_kind (opt_out_set|opt_out_cleared|retention_set|",
    "                          policy_deleted). With --after-id: cursor forward in time",
    "                          (newer entries since diff-timeline is ASC). With --before-id:",
    "                          cursor backward toward older entries. --after-id and",
    "                          --before-id are mutually exclusive. Output gives nextAfterId",
    "                          + nextBeforeId when page is full. (requires PG env)",
    ...
    "                          --actor-id filters to events authored by that actor. --kind",
    "                          filters to one event_kind. --after-id cursors forward,",
    "                          --before-id cursors backward (mutually exclusive). Output",
    "                          gives nextAfterId + nextBeforeId when page is full.",
    "                          (requires PG env)",

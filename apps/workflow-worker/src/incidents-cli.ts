...
/**
 * Parses `workflow-worker incidents <open|period|verify|metrics|ack|mitigate>
 * [options]` argv (the slice after the `incidents` verb). `period`/`verify`/
 * `metrics` require `--from` + `--to` (an ISO window); `ack`/`mitigate` require a
 * positional incident id. `--format human|json` (default human). Throws
 * `CliUsageError` on misuse. Produces the `IncidentsCliOptions` the
 * `@crossengin/incident-response-pg` runner consumes.
 */

...
export function parseIncidentsArgs(argv: readonly string[]): IncidentsCliOptions {
  ...
  if (first === "--help" || first === "-h" || first === undefined) {
    return { command: "open", from: null, to: null, limit: null, schema: null, format: "human", help: true };
  }
  if (!COMMANDS.has(first)) throw new CliUsageError(`unknown incidents command: ${first} (open|period|verify)`);
...
Options:
  ...
  --to <iso>       Window end (ISO timestamp; required for period/verify)

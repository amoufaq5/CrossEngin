...
export type SignalDeliveryGuarantee = (typeof SIGNAL_DELIVERY_GUARANTEES)[number];
...
export const canTransitionSignal = (from: SignalStatus, to: SignalStatus): boolean =>
  SIGNAL_TRANSITIONS[from].includes(to);
...
export const WorkflowSignalSchema = z
  ...
  .superRefine((s, ctx) => {
    if (s.deliveryGuarantee === "exactly_once_idempotent" && s.idempotencyKey === null) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["idempotencyKey"],
        message: "exactly_once_idempotent delivery requires idempotencyKey",
      });
    }
    ...
    if (s.status === "matched_to_instance") {
      if (s.instanceId === null || s.matchedAt === null) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["matchedAt"],
          message: "matched_to_instance status requires instanceId + matchedAt",
        });
      }
    }
    if (s.status === "consumed") {
      if (s.instanceId === null || s.matchedAt === null || s.consumedAt === null) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["consumedAt"],
          message: "consumed status requires instanceId + matchedAt + consumedAt",
        });
      }
    }
    ...
    if (s.expiresAt !== null && Date.parse(s.expiresAt) <= Date.parse(s.receivedAt)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["expiresAt"],
    ...
    if (s.matchedAt !== null && Date.parse(s.matchedAt) < Date.parse(s.receivedAt)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["matchedAt"],
...
export const isSignalExpired = (signal: WorkflowSignal, now: Date): boolean => {
...
export const findDuplicateSignal = (
  ...
  return (
    signals.find(
      (s) => s.signalName === candidate.signalName && s.idempotencyKey === candidate.idempotencyKey,
    ) ?? null
  );
};
...
export const matchSignalToInstance = (
  ...
  instances: readonly {
    ...
    awaitingSignalNames: readonly string[];
    tenantId: string;
  }[],
): string | null => {

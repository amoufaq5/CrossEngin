...
export interface WorkflowJobWorkerOptions {
  ...
  readonly claimer: JobClaimer;
  readonly processor: JobProcessor;
  /** Max job runs claimed per poll (default 20). */
...
/**
 * Polls a durable job-run queue and executes due runs, forever, until `stop()`. Many instances run
 * concurrently against the same queue — `claimDueJobs`' `FOR UPDATE SKIP LOCKED` gives each a disjoint
 * batch, so the fleet scales horizontally with no coordination. After a poll that found work it
 * re-polls almost immediately (drain the backlog); after an empty poll it backs off to `idlePollMs`.
 * A claim error is reported via `onError` and the loop continues (transient DB blips shouldn't kill a
 * worker). Mirror of `WorkflowActivityWorker` for the third P2 work type.
 */
export class WorkflowJobWorker {
  ...
  start(): void {
    ...
    this.loop = (async () => {
      while (this.running) {
        let result: JobBatchResult | null = null;
        try {

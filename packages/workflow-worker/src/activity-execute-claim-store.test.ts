...
import { PostgresActivityExecuteClaimStore } from "./activity-execute-claim-store.js";
...
describe("PostgresActivityExecuteClaimStore.claimScheduledActivities", () => {
  it("claims scheduled async activities with SKIP LOCKED + lease", async () => {
    const cap = capturePg([{ activity_id: "wfa_a", instance_ref: "wfi_1" }]);
    const store = new PostgresActivityExecuteClaimStore(cap.conn);
    const claims = await store.claimScheduledActivities({ workerId: "w1", now: NOW, limit: 10, leaseMs: 60_000 });
    expect(cap.last.sql).toContain("status = 'scheduled'");
    expect(cap.last.sql).toContain("execution_mode = 'async'");
  ...
  it("honors a custom schema and rejects an invalid one", async () => {
    const cap = capturePg();
    await new PostgresActivityExecuteClaimStore(cap.conn, { schema: "wf" }).claimScheduledActivities({ workerId: "w", now: NOW, limit: 1, leaseMs: 1000 });
    expect(cap.last.sql).toContain("wf.workflow_activities");
    expect(() => new PostgresActivityExecuteClaimStore(cap.conn, { schema: "x; DROP" })).toThrow(/invalid schema/);
  });

  it("releaseActivity clears the lease", async () => {
    const cap = capturePg();
    await new PostgresActivityExecuteClaimStore(cap.conn).releaseActivity("wfa_a");

...
export function buildErpCorePack(opts: BuildErpCorePackOptions = {}): Manifest {
  return {
    ...
    meta: {
      ...
      description:
        opts.description ??
        "Enterprise ERP core (51 entities): CRM (Account, Contact, Invoice), Sales/O2C (Lead, Opportunity, Quote, SalesOrder, Shipment), Inventory (Item, Warehouse, StockLevel, StockMovement), Manufacturing (BillOfMaterials, WorkOrder), Procurement (Vendor, PurchaseOrder, GoodsReceipt), Finance (LedgerAccount, JournalEntry, Payment, Expense, Bill, WhtCertificate), Accounting depth (Currency, ExchangeRate, FiscalYear, FiscalPeriod, AccountingBook for IFRS/US-GAAP/local-GAAP parallel books, CostCenter), country tax (TaxJurisdiction, TaxRule, TaxReturn for VAT/GST/withholding filing), Projects/Services (Project, ProjectTask, Timesheet), Assets (FixedAsset, MaintenanceOrder), Pricing/Tax (TaxCode, PriceList), and HR (Department, Position, Employee, LeaveRequest) — with 19 lifecycle workflows, RBAC across 15 roles, multi-currency GL postings, fiscal-period close, document auto-numbering, and 14 integration/automation jobs (payment-gateway sync, e-invoicing, tax engine, MRP reorder, depreciation, payroll disbursement, carrier tracking).",

...
describe("PostgresTraceRetention.diffHistoryEntries --actor-id-not exclusion check (M6.7.zz.tenant.opt-out.cli.diff-history.actor-not)", () => {
  const TENANT = "00000000-0000-4000-8000-00000000000A";
  const ID_A = "aa000000-0000-4000-8000-0000000000aa";
  const ID_B = "bb000000-0000-4000-8000-0000000000bb";
  const ACTOR_ALICE = "11111111-0000-4000-8000-000000000001";
  ...
  it("accepts when neither event has the excluded actor_id", async () => {
    const conn = mockConnection(() => ({
      rows: [
        rawEntry(ID_A, { actor_id: ACTOR_ALICE }),
        rawEntry(ID_B, { actor_id: ACTOR_BOB }),
  ...
  it("accepts when both events have null actor_id (system) and excluded is a UUID", async () => {
    ...
    const result = await r.diffHistoryEntries({
      idA: ID_A,
      idB: ID_B,
      actorIdNot: ACTOR_ALICE,
    });
    expect(result.idA).toBe(ID_A);
  });

  it("throws when event A matches the excluded actor", async () => {
    const conn = mockConnection(() => ({
      rows: [
    ...
    await expect(
      ...
      `expected neither event to have actor_id '${ACTOR_CAROL}' but A matches`,
    );
  });

  it("throws when event B matches the excluded actor", async () => {
    const conn = mockConnection(() => ({
      rows: [
        rawEntry(ID_A, { actor_id: ACTOR_ALICE }),
    ...
    await expect(
      ...
      `expected neither event to have actor_id '${ACTOR_CAROL}' but B matches`,
    );
  });

  it("throws naming both when both events match the excluded actor", async () => {
    const conn = mockConnection(() => ({
      rows: [
        rawEntry(ID_A, { actor_id: ACTOR_CAROL }),
        rawEntry(ID_B, { actor_id: ACTOR_CAROL }),
    ...
    await expect(
      ...
      `expected neither event to have actor_id '${ACTOR_CAROL}' but both A and B match`,
    );
  });

  it("omits the check when actorIdNot not set (backward compat)", async () => {
    const conn = mockConnection(() => ({
      rows: [
        rawEntry(ID_A, { actor_id: ACTOR_ALICE }),
        rawEntry(ID_B, { actor_id: ACTOR_BOB }),
...
describe("PostgresTraceRetention.listOptOutHistory actorId filter (M6.7.zz.tenant.opt-out.cli.history.actor-filter)", () => {
  ...
  it("adds h.actor_id = $N WHERE clause when actorId is set", async () => {
    const capture: Capture[] = [];
    const conn = mockConnection(() => ({ rows: [], rowCount: 0 }), capture);
    const r = new PostgresTraceRetention({ conn });
    await r.listOptOutHistory({ actorId: ACTOR_A });
    expect(capture[0]?.sql).toContain("h.actor_id = $1");
    expect(capture[0]?.params).toEqual([ACTOR_A, 100]);
  });

  it("omits actor_id WHERE clause when actorId is not set", async () => {
    const capture: Capture[] = [];
    const conn = mockConnection(() => ({ rows: [], rowCount: 0 }), capture);
    const r = new PostgresTraceRetention({ conn });
    await r.listOptOutHistory({});
  ...
  it("composes with other filters (tenantId + actorId)", async () => {
    const capture: Capture[] = [];
    const conn = mockConnection(() => ({ rows: [], rowCount: 0 }), capture);
    const r = new PostgresTraceRetention({ conn });
    await r.listOptOutHistory({
    ...
    expect(capture[0]?.sql).toContain("h.tenant_id = $1");
    expect(capture[0]?.sql).toContain("h.actor_id = $2");
  ...
  it("composes with all filter dimensions (tenant + table + kind + actor + since + until + afterId)", async () => {
    const capture: Capture[] = [];
    const conn = mockConnection(() => ({ rows: [], rowCount: 0 }), capture);
    const r = new PostgresTraceRetention({ conn });
    await r.listOptOutHistory({
  ...
  it("returns only rows matching the actor (substrate verifies WHERE filter)", async () => {
    const conn = mockConnection(() => ({
      rows: [
        {
          id: "h1",
    ...
    const r = new PostgresTraceRetention({ conn });
    const entries = await r.listOptOutHistory({ actorId: ACTOR_A });
    expect(entries).toHaveLength(1);
    expect(entries[0]?.actorId).toBe(ACTOR_A);
  });

  it("returns empty array when filtered actor has no rows", async () => {
    const conn = mockConnection(() => ({ rows: [], rowCount: 0 }));
    const r = new PostgresTraceRetention({ conn });
    const entries = await r.listOptOutHistory({ actorId: ACTOR_B });
...
describe("PostgresTraceRetention.listOptOutHistory actorIdsNot filter (M6.7.zz.tenant.opt-out.cli.history.actor-not + actor-not.multi)", () => {
  ...
  it("composes with actorId (positive + negative both present — contradictory but adapter doesn't enforce)", async () => {
    const capture: Capture[] = [];
    const conn = mockConnection(() => ({ rows: [], rowCount: 0 }), capture);
    const r = new PostgresTraceRetention({ conn });
    await r.listOptOutHistory({

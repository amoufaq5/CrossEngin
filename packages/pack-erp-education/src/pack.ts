 ...
 * `meta.extends: ["operate-erp/core"]`. It references the core `Account`
 * entity by name, so it cross-validates only once resolved against a
 * registry that supplies the core pack.
 */
export function buildErpEducationPack(
  opts: BuildErpEducationPackOptions = {},
): Manifest {
  return {
    manifestVersion: "1.0",
    meta: {
      ...
      description:
        opts.description ??
        "Education vertical (Student, Course, Enrollment) extending ERP Core — ties student records to the core billing Account, with FERPA education records (student contact info + recorded grades) classified as PII.",
      extends: [ERP_CORE_PACK_SLUG],
      compliancePacks: [
        ...(opts.compliancePacks ?? DEFAULT_EDUCATION_COMPLIANCE_PACKS),
      ],
    },

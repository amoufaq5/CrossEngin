...
> end-to-end against real Postgres. ADRs 0001–0177 are drafted in
...
```
CrossEngin/
├── docs/             vision + ADRs 0001-0177  (CC BY 4.0)
│   ├── vision.md
...
- **Human contributor?** Start with **[`docs/vision.md`](docs/vision.md)**
  ...
  index of 177 architecture decisions). Individual ADRs live at
...
The 48 integration tests in `apps/workflow-worker` + `apps/operate-server`
...
CROSSENGIN_PG_TEST=1 PGHOST=localhost PGUSER=postgres PGPASSWORD=postgres \
  ...
  pnpm --filter @crossengin/workflow-worker-app test   # and operate-server, operate-web
```

...
> under a real schema.
> **56 packages + 1 app, 129 meta-schema tables, 9,700 tests**,
> all green, zero type errors — in src AND in `*.test.ts` files.
> ADRs 0001–0277 fully drafted.
...
pnpm -r typecheck                           # type-check all package src
pnpm typecheck:tests                         # type-check all *.test.ts (build + root pass)
pnpm lint                                    # ESLint v9 flat config (whole workspace)
pnpm format:check                            # Prettier --check (TS/JS source)
